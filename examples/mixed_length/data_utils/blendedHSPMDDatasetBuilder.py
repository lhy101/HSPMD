...
from .hspmdDataset import LowLevelDataset, HSPMDDataset
from .utils import Split, normalize


MidLevelDataset = Union[HSPMDDataset]
...
class BlendedHSPMDDatasetBuilder(object):
    def __init__(
        self, cls: Type[MidLevelDataset], sizes: List[int], config: BlendedHSPMDDatasetConfig,
    ):
        self.cls = cls
        self.sizes = sizes
        self.config = config
    ...
    def _build_blended_dataset_splits(self,) -> List[Optional[TopLevelDataset]]:
        ...
        if self.config.blend:
            ...
            if len(blend) == 1:
                return self._build_hspmd_dataset_splits(blend[0], split, self.sizes)
            ...
            hspmd_datasets = [[] for _ in range(len(Split))]

            for i in range(len(prefix_per_dataset)):
                hspmd_datasets_split = self._build_hspmd_dataset_splits(
                    prefix_per_dataset[i], split, sizes_per_dataset[i]
                )
                for j in range(len(hspmd_datasets_split)):
                    hspmd_datasets[j].append(hspmd_datasets_split[j])
            ...
            for i in range(len(hspmd_datasets)):
                is_none = map(lambda _: _ is None, hspmd_datasets[i])
        ...
        # Each split comes from a separate distribution
            ...
            for i in range(len(Split)):
                ...
                if len(blend) == 1:
                    blended_datasets.append(
                        self._build_hspmd_dataset_splits(blend[0], split_spoof, sizes_spoof)[i]
                    )
                ...
                # Blend consists of multiple weights and prefixes
                    ...
                    for j in range(len(prefix_per_dataset)):
                        hspmd_datasets.append(
                            self._build_hspmd_dataset_splits(
    ...
    def build_generic_dataset(
        ...
        try:
            ...
            log = (
                ...
                + f"the path_to_cache attribute in BlendedHSPMDDatasetConfig and "
