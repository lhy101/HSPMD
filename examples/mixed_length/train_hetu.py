...
from hspmd_llama import LLaMALMHeadModel
    ...
    def run_plan(
        ...
        if warm_up:
            for compute_strategy_id in compute_strategy_id_list:
                ...
                hspmd_train(feed_dict, num_micro_batches, compute_strategy_id, optimize_strategy_id, run_level=ht.run_level("compute_only") if compute_only else ht.run_level("update"))
        ...
        for step in range(args.begin_step, args.steps):
            ...
            if args.torch_profile != 0 and step == 0:
                with profile(activities=[ProfilerActivity.CUDA]) as prof:
                    results = hspmd_train(feed_dict, len(input_batch), optimal_compute_strategy_id, optimize_strategy_id, run_level=ht.run_level("compute_only") if compute_only else ht.run_level("update"))
