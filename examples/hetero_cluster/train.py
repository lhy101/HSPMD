...
from hspmd.utils.data import HSPMDJsonDataset, build_data_loader
from hspmd.utils.parallel import distributed_init, read_ds_parallel_config, parse_multi_ds_parallel_config

local_device = None
all_devices = None

def train_dataset_provider(args):
    train_dataset = HSPMDJsonDataset(
...
def pretrain(args):
    ...
    for i in range(len(input_ds_hierarchy)):
        ...
        config.multi_seq_lens_symbol.append([hspmd.IntSymbol(1) for _ in range(dcp_size)])
        config.multi_cp_group_symbol.append([hspmd.IntSymbol(1) for _ in range(dcp_size)])
    ...
    input_ids = hspmd.parallel_placeholder(hspmd.int64, global_shape=[dcp_size * 16], ds_hierarchy=input_ds_hierarchy, device_group_hierarchy=input_dg_hierarchy, name='input_ids')
    loss_mask =  hspmd.parallel_placeholder(hspmd.float32, global_shape=[dcp_size * 16], ds_hierarchy=label_ds_hierarchy, device_group_hierarchy=label_dg_hierarchy, name='loss_mask')
    masked_lm_labels = hspmd.parallel_placeholder(hspmd.int64, global_shape=[dcp_size * 16], ds_hierarchy=label_ds_hierarchy, device_group_hierarchy=label_dg_hierarchy, name='masked_lm_labels')
    ...
    def run_plan(
        ...
        for step in range(steps):
            ...
            if step == 2 and args.exp_file != "":
                os.environ['HSPMD_STRAGGLER'] = "EXP_NEW"
                os.environ['HSPMD_STRAGGLER_LOG_FILE'] = args.exp_file
            if args.torch_profile != 0 and step == 1:
                with profile(activities=[ProfilerActivity.CUDA]) as prof:
                    results = hspmd_train(feed_dict, num_micro_batches, strategy_id, run_level=run_level)
                prof.export_chrome_trace(f"trace/trace_{local_device}.json")
            else:
                results = hspmd_train(feed_dict, num_micro_batches, strategy_id, run_level=run_level)
            end_time = time.time()
            consumed_samples += global_batch_size
            if 'HSPMD_STRAGGLER_LOG_FILE' in os.environ:
    ...
    def test(): 
        ...
        consumed_samples = run_plan(
            ...
            run_level = hspmd.run_level("update") if args.compute_only == 0 else hspmd.run_level("compute_only")
        )
...
if __name__ == '__main__':
    ...
    with hspmd.graph("define_and_run", num_strategy=args.num_strategy):
        if args.bf16:
