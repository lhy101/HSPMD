...
class TestArithmeticOps(unittest.TestCase):
    ...
    def test_broadcast_div(self):
        ...
        for shape_x, shape_y in TestArithmeticOps._test_broadcast_shapes:
            ...
            if GRAD_TEST:
              ...
              torch_optimizer = optim.SGD([torch_in], lr = 0.5)
              hspmd_in = hspmd.Tensor(y_np, requires_grad=True)
              hspmd_x = hspmd.from_numpy(x_np)
              hspmd_out = hspmd.div(hspmd_x, hspmd_in)
              hspmd_loss = hspmd_out.sum()
              hspmd_optimizer = hspmd.SGDOptimizer([hspmd_in], lr = 0.5)
              torch_loss.backward()
              torch_optimizer.step()
    ...
    def test_rsqrt(self):
        ...
        for shape in TestArithmeticOps._test_elementwise_shapes:
            ...
            self.assertTrue(allclose(hspmd.rsqrt(x), gt))
    ...
    def test_pow(self):
        ...
        for shape in TestArithmeticOps._test_elementwise_shapes:
            ...
            for exponent in TestArithmeticOps._test_pow_exponents:
                ...
                self.assertTrue(allclose(hspmd.pow(x, exponent), gt))
                ...
                if GRAD_TEST:
                    ...
                    torch_optimizer = optim.SGD([torch_in], lr = 0.5)
                    hspmd_in = hspmd.Tensor(x_np, requires_grad=True)
                    hspmd_out = hspmd.pow(hspmd_in, exponent)
                    hspmd_loss = hspmd_out.sum()
                    hspmd_optimizer = hspmd.SGDOptimizer([hspmd_in], lr = 0.5)
                    torch_loss.backward()
                    torch_optimizer.step()
    ...
    def test_ceil(self):
        ...
        for shape in TestArithmeticOps._test_elementwise_shapes:
            ...
            self.assertTrue(allclose(hspmd.ceil(x), gt))
    ...
    def test_floor(self):
        ...
        for shape in TestArithmeticOps._test_elementwise_shapes:
            ...
            self.assertTrue(allclose(hspmd.floor(x), gt))
    ...
    def test_round(self):
        ...
        for shape in TestArithmeticOps._test_elementwise_shapes:
            ...
            self.assertTrue(allclose(hspmd.round(x), gt))
    ...
    def test_matmul_op(self):
        ...
        for shape_x, shape_y in TestMatMulOps._test_shapes:
            ...
            if GRAD_TEST:
              ...
              torch_optimizer = optim.SGD([torch_in], lr = 0.5)
              hspmd_in = hspmd.Tensor(x_np, requires_grad=True)
              hspmd_out = hspmd.matmul(hspmd_in, y)
              hspmd_loss = hspmd_out.sum()
              hspmd_optimizer = hspmd.SGDOptimizer([hspmd_in], lr = 0.5)
              torch_loss.backward()
              torch_optimizer.step()
    ...
    def test_concat_op(self):
        ...
        for shape in TestTransformOps._test_shapes:
            for dim in dims:
                ...
                y = hspmd.from_numpy(y_np)
                z = hspmd.from_numpy(z_np)
                self.assertTrue(allclose(hspmd.concat([x, y], dim), gt))
                gt = np.concatenate((x_np, y_np, z_np), dim)
                self.assertTrue(allclose(hspmd.concat([x, y, z], dim), gt))
                ...
                if GRAD_TEST:
                    ...
                    torch_optimizer = optim.SGD([torch_in], lr = 0.5)
                    hspmd_in = hspmd.Tensor(x_np, requires_grad=True)
                    hspmd_out = hspmd.concat([hspmd_in, y, z], dim)
                    hspmd_loss = hspmd_out.sum()
                    hspmd_optimizer = hspmd.SGDOptimizer([hspmd_in], lr = 0.5)
                    torch_loss.backward()
                    torch_optimizer.step()
...
#                 hspmd_loss = hspmd_out.sum()
#                 hspmd_optimizer = hspmd.SGDOptimizer(inputs_hspmd, lr = 0.5)
#                 hspmd_optimizer.minimize(hspmd_loss)
...
class TestDropoutOps(unittest.TestCase):
    ...
    def test_dropout_op(self):
        ...
        for shape in TestDropoutOps._test_shapes:
            ...
            p = 0.5
            hspmd_out = hspmd.dropout(x, p)

            hspmd_out_numpy = hspmd_out.numpy(force=True)
            nonzero_scale = np.mean(np.abs(hspmd_out_numpy[hspmd_out_numpy != 0] / x_np[hspmd_out_numpy != 0]))
    ...
    def test_dropout2d_op(self):
        ...
        for shape in TestDropoutOps._test_shapes:
            ...
            p = 0.5
            hspmd_out = hspmd.dropout2d(x, p)

            hspmd_out_numpy = hspmd_out.numpy(force=True)
            nonzero_channels = hspmd_out_numpy[~np.all(hspmd_out_numpy == 0, axis=(2,3))]
            if nonzero_channels.size > 0:
                scale = np.mean(np.abs(nonzero_channels / x_np[~np.all(hspmd_out_numpy == 0, axis=(2,3))]))
...
class TestOtherOps(unittest.TestCase):
    ...
    def test_maskedfillop(self):
        ...
        for shape_x in TestOtherOps._maskedfill_test_shapes:
            ...
            x = hspmd.from_numpy(x_np)
            mask = hspmd.from_numpy(mask_np.astype(np.int64))
    ...
    #         q = hspmd.from_numpy(q_np)
    #         k = hspmd.from_numpy(k_np)
    #         v = hspmd.from_numpy(v_np)
    ...
    #         k = hspmd.from_numpy(k_np)
    #         v = hspmd.from_numpy(v_np)
    #         attn_output2 = hspmd.attn(q, k, v)[0]
    ...
    def test_rotaryop(self):
        ...
        for shape_x, shape_cos in TestOtherOps._rotary_test_shapes:
            ...
            x = hspmd.from_numpy(x_np)
            cos = hspmd.from_numpy(cos_np)
            sin = hspmd.from_numpy(sin_np)
            ...
            self.assertTrue(allclose(hspmd.rotary(x, cos, sin), gt))
            ...
            if GRAD_TEST:
                ...
                torch_optimizer = optim.SGD([torch_x], lr = 0.5)
                hspmd_x = hspmd.Tensor(x_np, requires_grad=True)
                hspmd_out = hspmd.rotary(hspmd_x, cos, sin)
                hspmd_loss = hspmd_out.sum()
                hspmd_optimizer = hspmd.SGDOptimizer([hspmd_x], lr = 0.5)
                torch_loss.backward()
                torch_optimizer.step()
                hspmd_optimizer.minimize(hspmd_loss)
                self.assertTrue(allclose(hspmd_x, torch_x.cpu().detach().numpy()))
        print(sys._getframe().f_code.co_name)
    ...
    def test_rmsop(self):
        ...
        for shape_x, shape_cos in TestOtherOps._rms_test_shapes:
            ...
            x0 = hspmd.from_numpy(x0_np)
            residual = hspmd.from_numpy(residual_np)
            weight = hspmd.from_numpy(weight_np)
            bias = hspmd.from_numpy(bias_np)
            ...
            # self.assertTrue(allclose(hspmd.rms_norm(x0, residual, weight, bias, None, None, None, None,
            ...
            if GRAD_TEST:
                ...
                torch_loss = torch_out.sum()
                torch_optimizer = optim.SGD([torch_x], lr = 0.5)
                hspmd_x = hspmd.Tensor(x0_np, requires_grad=True)
                hspmd_out = hspmd.rms_norm(hspmd_x, residual, weight, bias, None, None, None, None,
                                         dropout_p=0., epsilon=1e-3,
                                         rowscale_const=1., z_numrows=0,
                                         residual_in_fp32=False, prenorm=False, 
