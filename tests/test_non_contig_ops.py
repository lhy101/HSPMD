...
class TestNonContigOps(unittest.TestCase):
    ...
    def test_as_strided_op(self):
        ...
        for shape_x, shape_y, stride, storage_offset in TestNonContigOps._asstrided_test_shapes:
            ...
            x = hspmd.from_numpy(x_np)
            y = hspmd.as_strided(x, list(shape_y), list(stride), storage_offset)
            self.assertTrue(allclose(y, gt))
            ...
            if GRAD_TEST:
                ...
                torch_optimizer = optim.SGD([torch_in], lr = 0.5)
                hspmd_in = hspmd.Tensor(x_np, requires_grad=True)
                hspmd_out = hspmd.as_strided(hspmd_in, list(shape_y), list(stride), storage_offset)
                hspmd_loss = hspmd_out.sum()
                hspmd_optimizer = hspmd.SGDOptimizer([hspmd_in], lr = 0.5)
                torch_loss.backward()
                torch_optimizer.step()
    ...
    def test_diagonal_op(self):
        ...
        for shape, offset, dim1, dim2 in TestNonContigOps._diagonal_test_args:
            ...
            x = hspmd.from_numpy(x_np)
            y = hspmd.diagonal(x, offset, dim1, dim2)
            self.assertTrue(allclose(y, gt))
            ...
            if GRAD_TEST:
                ...
                torch_optimizer = optim.SGD([torch_in], lr = 0.5)
                hspmd_in = hspmd.Tensor(x_np, requires_grad=True)
                hspmd_out = hspmd.diagonal(hspmd_in, offset, dim1, dim2)
                hspmd_loss = hspmd_out.sum()
                hspmd_optimizer = hspmd.SGDOptimizer([hspmd_in], lr = 0.5)
                torch_loss.backward()
                torch_optimizer.step()
