...
class TestArithmeticOps(unittest.TestCase):
    ...
    def test_elementwise_add(self):
        for shape in TestArithmeticOps._test_elementwise_shapes:
            x = hspmd.randn(shape, dtype=hspmd.bfloat16)
            y = hspmd.randn(shape, dtype=hspmd.bfloat16)
            x_np = x.to(hspmd.float32).numpy(force=True)
            y_np = y.to(hspmd.float32).numpy(force=True)
            c = np.random.randn()
    ...
    def test_broadcast_add(self):
        for shape_x, shape_y in TestArithmeticOps._test_broadcast_shapes:
            x = hspmd.randn(shape_x, dtype=hspmd.bfloat16)
            y = hspmd.randn(shape_y, dtype=hspmd.bfloat16)
            x_np = x.to(hspmd.float32).numpy(force=True)
            y_np = y.to(hspmd.float32).numpy(force=True)
            gt = x_np + y_np
            self.assertTrue(allclose(x + y, gt))
            ...
            if GRAD_TEST:
              ...
              hspmd_in = hspmd.Tensor(y_np, requires_grad=True, dtype=hspmd.bfloat16)
    ...
    def test_broadcast_div(self):
        for shape_x, shape_y in TestArithmeticOps._test_broadcast_shapes:
            ...
            if GRAD_TEST:
              ...
              hspmd_loss = hspmd_out.sum()
              hspmd_optimizer = hspmd.SGDOptimizer([hspmd_in], lr = 0.1)
              torch_loss.backward()
              torch_optimizer.step()
              hspmd_optimizer.minimize(hspmd_loss)
              self.assertTrue(allclose(hspmd_in, torch_in.detach().numpy()))
    ...
    def test_sum(self):
        for shape in TestArithmeticOps._test_elementwise_shapes:
            x = hspmd.randn(shape, dtype=hspmd.bfloat16)
            y = hspmd.randn(shape, dtype=hspmd.bfloat16)
            z = hspmd.randn(shape, dtype=hspmd.bfloat16)
            x_np = x.to(hspmd.float32).numpy(force=True)
            y_np = y.to(hspmd.float32).numpy(force=True)
            z_np = z.to(hspmd.float32).numpy(force=True)
            gt = x_np + y_np + z_np
    ...
    def test_matmul_op(self):
        for shape_x, shape_y in TestMatMulOps._test_shapes:
            ...
            if GRAD_TEST:
              ...
              torch_optimizer.step()
              hspmd_optimizer.minimize(hspmd_loss)
              self.assertTrue(allclose(hspmd_in, torch_in.cpu().detach().numpy()))
    ...
    def test_conv2d_op(self):
        for shape in TestConv2dOps._data_shapes:
            ...
            for f_shape in TestConv2dOps._filter_shapes:
                f = hspmd.randn(f_shape, dtype=hspmd.bfloat16)
                f_np = f.to(hspmd.float32).numpy(force=True)
                ...
                bias = hspmd.randn(bias_shape, dtype=hspmd.bfloat16)
                bias_np = bias.to(hspmd.float32).numpy(force=True)
    ...
    def test_layernorm_op(self):
        for shape in TestPoolOps._test_shapes:
            for i in range(1, 4):
                ...
                x = hspmd.randn(shape, dtype=hspmd.bfloat16)
                x_np = x.to(hspmd.float32).numpy(force=True)
                scale = hspmd.ones(norm_shape, dtype=hspmd.bfloat16)
                scale_np = scale.to(hspmd.float32).numpy(force=True)
                bias = hspmd.randn(norm_shape, dtype=hspmd.bfloat16)
                bias_np = bias.to(hspmd.float32).numpy(force=True)
...
class TestLossOps(unittest.TestCase):
    ...
    def test_bce_op(self):
        ...
        for shape in TestLossOps._test_binary_label_shapes:
            ...
            probs = hspmd.from_numpy(probs_np).to(hspmd.bfloat16)
            probs_np = probs.to(hspmd.float32).numpy(force=True)
            labels_np = np.random.choice([0, 1], size=shape).astype(np.float32)
            labels = hspmd.from_numpy(labels_np).to(hspmd.bfloat16)
            labels_np = labels.to(hspmd.float32).numpy(force=True)
    ...
    def test_softmax_cross_entropy_op(self):
        ...
        for shape in TestLossOps._test_cross_entropy_label_shapes:
            ...
            probs = hspmd.from_numpy(probs_np).to(hspmd.bfloat16)
            labels = hspmd.from_numpy(labels_onehot).to(hspmd.bfloat16)
            ...
            if GRAD_TEST:
                ...
                hspmd_loss = hspmd_out.sum()
                hspmd_optimizer = hspmd.SGDOptimizer([hspmd_in], lr = 2000)
                torch_loss.backward()
                torch_optimizer.step()
                print(hspmd_in.numpy(force=True))
                hspmd_optimizer.minimize(hspmd_loss)
                print(hspmd_in.numpy(force=True))
    ...
    def test_einsum_op_simple(self):
        ...
        for equation, nshapes in TestEinsumOps._test_args:
            ...
            for shape in nshapes:
                ...
                input_hspmd = hspmd.from_numpy(input_np).to(hspmd.bfloat16)
                input_np = input_hspmd.to(hspmd.float32).numpy(force=True)
...
class TestOtherOps(unittest.TestCase):
    ...
    def test_whereop(self):
        ...
        for shape_x in TestOtherOps._onehot_test_shapes:
            ...
            x = hspmd.randn(shape_x, dtype=hspmd.bfloat16)
            x_np = x.to(hspmd.float32).numpy(force=True)
            y = hspmd.randn(shape_x, dtype=hspmd.bfloat16)
            y_np = y.to(hspmd.float32).numpy(force=True)
