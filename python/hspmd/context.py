import _hspmd_core as ht_core
import builtins # bool is resovled as hspmd.bool
...
class _GraphContext(object):
    def __init__(self, g, create_new=False, prefix="default", num_strategy=-1, tmp=False):
        ...
        if isinstance(g, ht_core.Graph):
            assert create_new == False, f"Using type {type(g).__name__} to create a hspmd.Graph is not allowed, please use a str instead."
            self.graph = g
            ...
            raise ValueError(f"Cannot parse type '{type(g).__name__}' as hspmd.Graph")
        if num_strategy >= 1:
...
class _RunLevel(object):
    def __init__(self, type):
        if type == "update":
            ...
            raise ValueError(f"No level for '{type}' in hspmd.Graph")
