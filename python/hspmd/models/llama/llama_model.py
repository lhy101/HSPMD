...
class LLamaModel(hspmd.nn.Module):
    ...
    def forward(
        ...
        if sp:
            ...
            hidden_states = hspmd.reshard(hidden_states, ds_hierarchy_output, name="workaround_sp_scatter")
...
class LLaMALMHeadModel(hspmd.nn.Module):
    ...
    def forward(
        ...
        if not hidden_states.check_ds_hierarchy_equal(self.lm_head.ds_union_map['split0_dup']):
            hidden_states = hspmd.reshard(hidden_states, self.lm_head.ds_union_map['split0_dup'])
        '''
