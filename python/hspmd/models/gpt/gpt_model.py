...
# self-attn
class GPTAttention(hspmd.nn.Module):
    ...
    def _attn(self, query, key_t, value, attention_mask=None):
        ...
        attn_weights = hspmd.bmm(query, key_t)
        ...
        causal_mask = hspmd.from_numpy_parallel(parallel_multi_data_provider(
        ...
        # softmax
        attn_weights = hspmd.softmax(attn_weights, 3)
        # dropout
        # attn_weights = self.attn_dropout(attn_weights)
        # weight sum, shape=[micro_batch_size, num_heads, seq_len, head_dim]
        attn_output = hspmd.bmm(attn_weights, value)
    ...
    def forward(
        ...
        hidden_states = hidden_states.reshape([self.config.mbs_times_dp_symbol * self.config.seq_len_symbol, hspmd.IntSymbol(embed_dim)])
...
class ParallelMLP(hspmd.nn.Module):
    def __init__(self, config, ds_parallel_configs, layer_idx, name='mlp'):
        ...
        # self.activation_func = hspmd.nn.NewGeLU()
    ...
    def forward(self, hidden_states):
        ...
        intermediate_parallel = hspmd.relu(intermediate_parallel)
...
class GPTMLP(hspmd.nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if len(origin_shape) != 2: # shape adaptor
            hidden_states = hidden_states.reshape([self.config.mbs_times_dp_symbol * self.config.seq_len_symbol, hspmd.IntSymbol(origin_shape[-1])])
...
class GPTBlock(hspmd.nn.Module):
    def __init__(self, config, ds_parallel_configs, layer_idx):
        ...
        self.ln_1 = hspmd.nn.HtMultiParallelLayerNorm(hidden_size, get_multi_ds_parallel_config(ds_parallel_configs, 'layernorm1', layer_idx), eps=config.layer_norm_epsilon, name=f'ln1_block{layer_idx}')
        self.attn = GPTAttention(config, ds_parallel_configs, layer_idx=layer_idx, name=f'attn_block{layer_idx}')
        self.ln_2 = hspmd.nn.HtMultiParallelLayerNorm(hidden_size, get_multi_ds_parallel_config(ds_parallel_configs, 'layernorm2', layer_idx), eps=config.layer_norm_epsilon, name=f'ln2_block{layer_idx}')
    ...
    def forward(
        ...
        hidden_states = hspmd.reshard(hidden_states, self.attn.qkv_dense.ds_union_map['split0_dup'], self.ln_1.device_group_unions, name=f"pipeline_layer_{self.layer_idx}")
...
class GPTModel(hspmd.nn.Module):
    def __init__(self, config, ds_parallel_configs):
        ...
        self.ln_f = hspmd.nn.HtMultiParallelLayerNorm(self.embed_dim, get_multi_ds_parallel_config(ds_parallel_configs, 'layernorm_final'), eps=config.layer_norm_epsilon, name='ln_final')
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attention_mask = attention_mask.reshape([self.config.mbs_times_dp_symbol, hspmd.IntSymbol(1), hspmd.IntSymbol(1), self.config.seq_len_symbol])
...
class GPTLMHeadModel(hspmd.nn.Module):
    ...
    def __init__(self, config, ds_parallel_configs):
        ...
        self.lm_head.weight = hspmd.reshard(self.transformer.wte.embedding_table, self.lm_head.ds_union_map['dup_split0'], self.lm_head.device_group_unions, name="share_weight_comm") 
        self.config = config
    ...
    def forward(
        ...
        shift_hidden_states = hspmd.slice(hidden_states, [hspmd.IntSymbol(0), hspmd.IntSymbol(0), hspmd.IntSymbol(0)], [hidden_states.symbolic_shape[0], hidden_states.symbolic_shape[1] - 1, hidden_states.symbolic_shape[2]])
        _,  _, n_embd = hidden_states.global_shape
        # [b*(seq_len-1), n_embd]
        shift_hidden_states = shift_hidden_states.reshape([self.config.mbs_times_dp_symbol * (self.config.seq_len_symbol - 1), hspmd.IntSymbol(n_embd)])
        ...
        if labels is not None:
            ...
            shift_labels = hspmd.slice(labels, [hspmd.IntSymbol(0), hspmd.IntSymbol(1)], [labels.symbolic_shape[0], labels.symbolic_shape[1] - 1])
            ...
            loss = hspmd.vocab_parallel_cross_entropy(shift_lm_logits,
