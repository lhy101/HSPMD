...
WEIGHTS_NAME = 'hspmd_pytorch_model'
...
def need_transfer(src_dtype, dst_dtype):
    if dst_dtype is None:
        return False
    if (src_dtype != hspmd.float32) and \
       (src_dtype != hspmd.float16) and \
       (src_dtype != hspmd.bfloat16):
        return False
...
def need_quantization(src_dtype, dst_dtype):
    if (not need_transfer(src_dtype, dst_dtype)):
        return False
    if (dst_dtype != hspmd.float4) and (dst_dtype != hspmd.nfloat4):
        return False
    return True
...
def data_transform_for_store(key, param, value, config, save_dtype, ht_state_dict, local_device):
    ...
    flag = 0
    if (param.dtype == hspmd.float4) or (param.dtype == hspmd.nfloat4):
        flag += 1
        key_absmax = key + "_absmax"
        weight_absmax = ht_state_dict[key_absmax]
        global_value = hspmd.dequantization(local_value, weight_absmax, 
                                           config.dqtype, 64,
    ...
    if key_lora_A in ht_state_dict:
        global_value = global_value + hspmd.matmul(ht_state_dict[key_lora_A], 
    ...
    if not param.distributed_states.is_pure_duplicate:
        ...
        global_value = hspmd.reshard(global_value, ds_dup)
...
def save_model(
    model: hspmd.nn.Module, filename: str, config=None, 
    ...
    with hspmd.graph("define_and_run", create_new=True, prefix="save_model"):
        ...
        ht_state_dict = model.state_dict(format='hspmd')
        ...
        for key, data in state_dict.items():
            ...
            if ((save_dtype == hspmd.float4) or (save_dtype == hspmd.nfloat4)) and \
...
def temp_save(
    model: hspmd.nn.Module, optimizer, filename: str, config=None, 
    local_device=None, save_dtype = None,
    force_contiguous: bool = False,
    only_lora: bool = False,
    metadata: Optional[Dict[str, str]] = None
    ...
    with hspmd.graph("define_and_run", create_new=True, prefix="save_model"):
        ...
        for i, device_group in enumerate(all_device_groups):
            if device_group.contains(local_device):
                ...
                local_losthame = hspmd.device.get_local_hostname()
...
def load_model(model: hspmd.nn.Module, filename: Union[str, os.PathLike],
               config=None, local_device=None, strict=True) -> Tuple[List[str], List[str]]:
    local_state_dict = model.state_dict(format='hspmd')
    hspmd_state_dict = model.state_dict(format='hspmd')
    all_device_groups = []
    parameter_to_dtype = {}
     
    # for k in local_state_dict:
    #     param = hspmd_state_dict[k]
    ...
    for k in local_state_dict:
        ...
        for i, archive in enumerate(archive_keys):
            if k in archive_keys[i]:
                state_dict[k] = hspmd.numpy_to_NDArray(archive_opens[i].get_tensor(k), parameter_to_dtype[k])
                # break
        # if ac_idx == 0:
        ...
        #     state_dict[k] = hspmd.numpy_to_NDArray(archive_opens[ac_idx].get_tensor(k), parameter_to_dtype[k])
...
def temp_load(model: hspmd.nn.Module, optimizer, filename: Union[str, os.PathLike],
              config=None, local_device=None, strict=True) -> Tuple[List[str], List[str]]:
    import time
    save_st = time.time()
    ...
    ds_files = []
    archive_files = []
    local_hostname = hspmd.device.get_local_hostname()
    for file in file_list:
            ...
            for state_k, state_param in opt_state_dict.items():
                ...
                for i in range(len(ds_json)):
                    if state_key in ds_json[i]:                
                        ...
                        break
                state_key = k + "_" + state_k
                hspmd_state_dict[state_key] = state_param
    ...
    if need_switch:
        ...
        for k in hspmd_state_dict:
            ...
            from hspmd.nn.modules.module import parallel_data_provider
            # global_data = hspmd.numpy_to_NDArray(split_numpys[0], parameter_to_dtype[k])
    ...
    for k in hspmd_state_dict:
        if k in state_dict:
            # print("Reset:", k)
            hspmd_state_dict[k].reset_data(state_dict[k])
    save_ed = time.time()
    print(local_device, 'Reset_Params_Time = %.4f'%(save_ed - save_st))
    return model

def temp_load_split(model: hspmd.nn.Module, optimizer, filename: Union[str, os.PathLike],
                    config=None, local_device=None, strict=True) -> Tuple[List[str], List[str]]:
    import time
    save_st = time.time()
    ...
    if need_switch:
        ...
        for k in hspmd_state_dict:
            ...
            state_dict[k] = hspmd.numpy_to_NDArray(split_data[0], parameter_to_dtype[k])
        ...
        with safe_open(archive_file, framework="np") as f:
            for k, dtype in parameter_to_dtype.items():
                ...
                split_dims = min(hspmd_state_dict[k].ndim, SPLIT_DIMS)
                for dim in range(split_dims - 1, -1, -1):
                    temp_split_data = []
                    split_dim = state["states"][dim] if dim in state["states"] else 1
                    temp_splits = max(split_dim, TEMP_SPLITS)
                    temp_splits = min(hspmd_state_dict[k].shape[dim], temp_splits)
                    splits_per_device = temp_splits // split_dim
                    num_groups = len(split_data) // splits_per_device
                    # print(local_device, k, state["states"], num_groups, dim, temp_splits, split_dim, splits_per_device)
                    for i in range(num_groups):
    ...
    with safe_open(filename, framework="np") as f:
        for k in f.keys():
            ...
            if (k in parameter_to_dtype):
                ...
                result[k] = hspmd.numpy_to_NDArray(f.get_tensor(k), parameter_to_dtype[k])
    return result
...
def _view2torch(safeview) -> Dict[str, torch.Tensor]:
    ...
    for k, v in safeview:
        ...
        arr = hspmd.frombuffer(v["data"], dtype=dtype).reshape(v["shape"])
        result[k] = arr
...
def _flatten(tensors: Dict[str, np.ndarray]) -> Dict[str, Dict[str, Any]]:
    if not isinstance(tensors, dict):
        raise ValueError(f"Expected a dict of [str, hspmd.NDArray] but received {type(tensors)}")

    for k, v in tensors.items():
        if not isinstance(v, np.ndarray):
            raise ValueError(f"Key `{k}` is invalid, expected hspmd.NDArray but received {type(v)}")
