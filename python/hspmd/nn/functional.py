...
def generalized_xavier_(tshape: List, dist: str, mode: str, gain: float, requires_grad: bool = False, dtype = hspmd.float32, device_groups = None) -> Tensor:
    factor = _calculate_correct_fan(tshape, mode)
    if dist == 'uniform':
        limit = math.sqrt(gain / factor)
        return hspmd.rand(tshape, -limit, limit, requires_grad = requires_grad, dtype = dtype, device_groups = device_groups)
    elif dist == 'normal':
        std = math.sqrt(gain / factor)
        return hspmd.randn(tshape, 0., std, requires_grad = requires_grad, dtype = dtype, device_groups = device_groups)
    else:
...
def kaiming_uniform_(tshape: List, a: float = 0., mode: str = 'fan_in', 
                     ...
                     dtype = hspmd.float32, device_groups = None) -> Tensor:
