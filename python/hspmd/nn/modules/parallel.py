...
class ParallelLayerNorm(Module):
    def __init__(self, normalized_shape, device_group, eps=1e-5, dtype=hspmd.float32, name='ln'):
    ...
    def forward(self, input_p):
        return hspmd.fused_layernorm(input_p, self.weight, self.bias, self.normalized_shape, self.eps, device_group=self.device_group, name=self.name)[0]

class ParallelEmbedding(Module):
    def __init__(self, num_embeddings, embedding_dim, device_group, init_method='xavier_normal_', dtype=hspmd.float32, name='embedding'):
...
class VocabParallelEmbedding(Module):
    def __init__(self, num_embeddings, embedding_dim, device_group, dp=1, fixed_vocab_start_index=True, init_method='xavier_normal_', dtype=hspmd.float32, name='vocab_embedding'):
        ...
        ds_split0_dup = hspmd.DistributedStates(num_devices, {-1: num_devices//dp, 0: dp}, [0, -1]) # for data
        ds_dup_split0 = hspmd.DistributedStates(num_devices, {-1: dp, 0: num_devices//dp}, [-1, 0]) # for embedding table
...
class ColumnParallelLinear(Module):
    ...
    def __init__(self, in_features, out_features, device_group, dp=1,
                 bias=True, gather_output=True, init_method='xavier_normal_', dtype=hspmd.float32, name='colp'):
        ...
        ds_dup_split0 = hspmd.DistributedStates(num_devices, {-1: dp, 0: num_devices//dp}, [-1, 0]) # for weights
        ds_split0_dup = hspmd.DistributedStates(num_devices, {-1: num_devices//dp, 0: dp}, [0, -1]) # for data
    ...
    def forward(self, input_p):
        ...
        if not self.gather_output:
            ...
            # bias = hspmd.reshard(self.bias, self.ds_map['split0_dup']) # for fusion
        return output
...
class RowParallelLinear(Module):
    ...
    """
    def __init__(self, in_features, out_features, device_group, dp=1,
                 bias=True, init_method='xavier_normal_', dtype=hspmd.float32, name='rowp'):
        ...
        ds_dup_split1 = hspmd.DistributedStates(num_devices, {-1: dp, 1: num_devices//dp}, [-1, 1]) # for weight
        ds_dup = hspmd.DistributedStates(num_devices, {-1: num_devices}, [-1]) # for bias
        ds_split01 = hspmd.DistributedStates(num_devices, {0: dp, 1: num_devices//dp}, [0, 1]) # for data split in dimension 1
        ds_split0_dup = hspmd.DistributedStates(num_devices, {-1: num_devices//dp, 0: dp}, [0, -1]) # for data reduce partial to dup
    ...
    def forward(self, input_p):
        ...
        tensor_split0_dup = hspmd.reshard(tensor_split0_partial, self.ds_map['split0_dup'])
