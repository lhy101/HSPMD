...
def config2ds(config):
    ...
    ds = hspmd.DistributedStates(num_devices, states, order, zero)
    
    all_devices = hspmd.global_device_group()
    device_group = hspmd.DeviceGroup([all_devices.get(device_id) for device_id in config['device_group']])
    return ds, device_group

class HtParallelRMSNorm(Module):
    def __init__(self, normalized_shape, ds_parallel_config, sequence_parallel=False, dtype=hspmd.float32, name='rmsnorm'):
        ...
        self.ds_split0 = hspmd.DistributedStates(ds.device_num, {0: ds.device_num}, [0], False) # for activation, no zero
    ...
    def forward(self, input_p):
        ...
        if self.sequence_parallel:
            ...
            output_rms = hspmd.rms_norm(input_p, None, self.weight, None, is_rms_norm=True, \
...
class HtParallelLayerNorm(Module):
    def __init__(self, normalized_shape, ds_parallel_config, sequence_parallel=False, eps=1e-5, 
                 dtype=hspmd.float32, name='ln', ori_model = None):
    ...
    def forward(self, input_p):
        ...
        if self.sequence_parallel:
            ...
            output_ln = hspmd.fused_layernorm(input_p, self.weight, self.bias, self.normalized_shape, \
...
class HtParallelEmbedding(Module):
    def __init__(self, num_embeddings, embedding_dim, ds_parallel_config, init_method='xavier_normal_', 
                 dtype=hspmd.float32, name='embedding', ori_model = None):
    ...
    def forward(self, input_p):
        return hspmd.embedding_lookup(self.embedding_table, input_p, device_group=self.device_group, name=self.name)
    
class HtVocabParallelEmbedding(Module):
    def __init__(self, num_embeddings, embedding_dim, ds_parallel_config, init_method='xavier_normal_', 
                 dtype=hspmd.float32, name='vocab_embedding', ori_model = None):
        ...
        device_index = get_device_index(self.device_group)
        ds_split0_dup = hspmd.DistributedStates(num_devices, {-1: tp, 0: dp}, [0, -1]) # for data
        ds_dup = hspmd.DistributedStates(num_devices, {-1: dp * tp}, [-1])
    ...
    def forward(self, input_p):
        if input_p.distributed_states.check_equal(self.ds_map['split0_dup']):
            ...
            tensor_split0_dup = hspmd.reshard(input_p, self.ds_map['split0_dup'])
        ...
        lookup_split0_partial = hspmd.embedding_lookup(self.embedding_table, input_offset, device_group=self.device_group, name=self.name)
        if lookup_split0_partial.distributed_states.check_equal(self.ds_map['split0_dup']): # pure dp
            output = lookup_split0_partial
        else:
            output = hspmd.reshard(lookup_split0_partial, self.ds_map['split0_dup'])
        return output
...
class HtColumnParallelLinear(Module):
    ...
    def __init__(self, in_features, out_features, ds_parallel_config,
                 ...
                 dtype=hspmd.float32, name='colp', ori_model = None):
        ...
        ds_dup_split0 = hspmd.DistributedStates(num_devices, {-1: dp, 0: tp}, [-1, 0], zero) # for weights with trans_b
        ds_split0_dup = hspmd.DistributedStates(num_devices, {-1: tp, 0: dp}, [0, -1]) # for data
        ds_dup = hspmd.DistributedStates(num_devices, {-1: dp * tp}, [-1], zero)
    ...
    def forward(self, input_p):
        ...
        #tensor_split01 = hspmd.linear(tensor_split0_dup, self.weight, self.bias, trans_b=True, device_group=self.device_group, name=self.name)
        tensor_split01 = hspmd.linear(tensor_split0_dup, self.weight, trans_b=True, device_group=self.device_group, name=self.name)
        if not self.gather_output:
            ...
            if tensor_split01.distributed_states.check_equal(self.ds_map['split0_dup']): # pure dp
                ...
                output = hspmd.reshard(tensor_split01, self.ds_map['split0_dup'])
...
class HtRowParallelLinear(Module):
    ...
    def __init__(self, in_features, out_features, 
                 ...
                 dtype=hspmd.float32, name='rowp', ori_model = None):
        ...
        device_index = get_device_index(self.device_group)
        # assume num_devices=8, there exists 4 cases: dp=1 tp=8, dp=2 tp=4, dp=4 tp=2, dp=8 tp=1
        ds_dup_split1 = hspmd.DistributedStates(num_devices, {-1: dp, 1: tp}, [-1, 1], zero) # for weight with trans_b
        ds_dup = hspmd.DistributedStates(num_devices, {-1: num_devices}, [-1], zero) # for bias
        ds_split01 = hspmd.DistributedStates(num_devices, {0: dp, 1: tp}, [0, 1]) # for data split in dimension 1
        ds_split0_dup = hspmd.DistributedStates(num_devices, {-1: tp, 0: dp}, [0, -1]) # for data reduce partial to dup
        ds_split0 = hspmd.DistributedStates(num_devices, {0: num_devices}, [0]) # for sequence parallel
    ...
    def forward(self, input_p):
        if input_p.distributed_states.check_equal(self.ds_map['split01']):
            ...
            tensor_split01 = hspmd.reshard(input_p, self.ds_map['split01'])

        tensor_split0_partial = hspmd.linear(tensor_split01, self.weight, trans_b=True, device_group=self.device_group, name=self.name)
        if tensor_split0_partial.distributed_states.check_equal(self.ds_map['split0_dup']): # pure dp
            output = tensor_split0_partial
        else:
            if self.sequence_parallel:
                output = hspmd.reshard(tensor_split0_partial, self.ds_map['split0']) # reduce-scatter
            else:
                output = hspmd.reshard(tensor_split0_partial, self.ds_map['split0_dup']) # allreduce
