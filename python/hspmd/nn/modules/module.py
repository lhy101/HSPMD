...
from hspmd import _tensor_type_t
...
def parallel_data_provider(global_data, ds, device_index):
    ...
    for dim in order:
        ...
        if isinstance(local_data, hspmd.NDArray): 
            begin_pos[dim] = start
...
class Module(object):
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        with hspmd.graph("define_and_run"):
            ...
            # Parameters
            if isinstance(value, hspmd.Tensor):
            ...
            # Modules
            if isinstance(value, hspmd.nn.Module):
    ...
    def __call__(self, *input, **kwargs) -> Any:
        with ExitStack() as stack:
            stack.enter_context(hspmd.graph("define_and_run"))
            if self.module_name is None:
                ...
                stack.enter_context(hspmd.subgraph(name = self.module_name, module_type = self.__class__.__name__))
                _parameters = self.__dict__.get('_parameters')
                for key, param in _parameters.items():
                    # print(self.module_name, " ", key, " ", type(param), param)
                    hspmd.add_to_subgraph(param)
            ...
            stack.enter_context(hspmd.recompute(mutli_recompute))
            stack.enter_context(hspmd.cpu_offload(mutli_cpu_offload))
    ...
    def _save_to_state_dict(self, destination, prefix, format='numpy'):
        ...
        submodule in :meth:`~hspmd.nn.Module.state_dict`.
        ...
        for name, param in _parameters.items():
            if param is not None:
                if format == 'numpy':
                    ...
                    raise NotImplementedError("state_dict() can only use numpy.ndarray or hspmd.Tensor.")
        _buffers = self.__dict__.get('_buffers')
    ...
    def _load_from_state_dict(self, state_dict, local_device, prefix, strict,
        ...
        in :meth:`~hspmd.nn.Module.load_state_dict`.

        .. note::
            :attr:`state_dict` is not the same object as the input
            :attr:`state_dict` to :meth:`~hspmd.nn.Module.load_state_dict`. So
            it can be modified.
    ...
    def load_state_dict(self, state_dict, local_device = None, strict = True):
        ...
        by this module's :meth:`~hspmd.nn.Module.state_dict` function.
