...
#include "hspmd/_binding/graph/tensor_ctor.h"
...
//       << "Type " << cls_type->tp_name << " is not derived from hspmd.SGDOptimizer";
...
//       << "Subclass of hspmd.SGDOptimizer must be created from a variable. "
...
std::vector<PyMethodDef> InitSGDOptimizerPyMethodDefs() {
  ...
  AddPyMethodDefs(ret, hspmd::graph::get_registered_optimizer_methods());
  return ret;
}
...
std::vector<PyMethodDef> InitSGDOptimizerPyClassMethodDefs() {
  ...
  AddPyMethodDefs(ret, hspmd::graph::get_registered_optimizer_class_methods());
  return ret;
}
