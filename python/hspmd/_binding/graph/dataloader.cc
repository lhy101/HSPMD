...
//       << "Type " << cls_type->tp_name << " is not derived from hspmd.Dataloader";
...
//       << "Subclass of hspmd.Dataloader must be created from a variable. "
...
std::vector<PyMethodDef> InitDataloaderPyMethodDefs() {
  ...
  AddPyMethodDefs(ret, hspmd::graph::get_registered_dataloader_methods());
  return ret;
}
...
std::vector<PyMethodDef> InitDataloaderPyClassMethodDefs() {
  ...
  AddPyMethodDefs(ret, hspmd::graph::get_registered_dataloader_class_methods());
  return ret;
}
