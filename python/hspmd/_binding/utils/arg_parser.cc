...
#include "hspmd/_binding/utils/python_primitives.h"
#include "hspmd/_binding/utils/numpy.h"
#include "hspmd/utils/string_utils.h"
...
std::string ArgType2Str(ArgType type) {
  switch (type) {
    ...
    case ArgType::DEVICE_GROUP:
      return "hspmd.DeviceGroup";
    case ArgType::DEVICE_GROUP_LIST:
      return "List[hspmd.DeviceGroup]";
    case ArgType::DG_HIERARCHY:
      return "List[List[hspmd.DeviceGroup]]";
    case ArgType::STREAM:
      return "hspmd.stream";
    case ArgType::ND_ARRAY:
      return "hspmd.NDArray";
    case ArgType::ND_ARRAY_LIST:
      return "List[hspmd.NDArray]";
    case ArgType::TENSOR:
      return "hspmd.Tensor";
    case ArgType::TENSOR_LIST:
      return "List[hspmd.Tensor]";
    case ArgType::OPERATOR:
      return "hspmd.Operator";
    case ArgType::OPERATOR_LIST:
      return "List[hspmd.Operator]";
    ...
    case ArgType::DISTRIBUTED_STATES:
      return "hspmd.DistributedStates";
    case ArgType::DISTRIBUTED_STATES_LIST:
      return "List[hspmd.DistributedStates]";
    case ArgType::DS_HIERARCHY:
      return "List[hspmd.DistributedStatesUnion]";
    case ArgType::INT_SYMBOL:
      return "hspmd.IntSymbol";
    case ArgType::SYMBOLIC_SHAPE:
      return "List[hspmd.IntSymbol]";
    case ArgType::SYMBOLIC_SHAPE_LIST:
      return "List[List[hspmd.IntSymbol]]";
    case ArgType::INITIALIZER:
      return "hspmd.Initializer";
    default:
...
ArgType Str2ArgType(const std::string& type) {
  ...
  if (type == "hspmd.dtype" || type == "dtype" || type == "DataType")
    return ArgType::DATA_TYPE;
  if (type == "hspmd.device" || type == "device" || type == "Device") 
    return ArgType::DEVICE;
  if (type == "hspmd.DeviceGroup" || type == "DeviceGroup")
    return ArgType::DEVICE_GROUP;
  if (type == "List[hspmd.DeviceGroup]" || type == "List[DeviceGroup]" ||
      type == "hspmd.DeviceGroupList" || type == "DeviceGroupList")
    return ArgType::DEVICE_GROUP_LIST;
  if (type == "List[List[hspmd.DeviceGroup]]" || type == "List[List[DeviceGroup]]" || 
      type == "DeviceGroupHierarchy")
    return ArgType::DG_HIERARCHY;
  if (type == "hspmd.stream" || type == "stream" || type == "Stream") 
    return ArgType::STREAM;
  if (type == "hspmd.NDArray" || type == "NDArray" || 
      type == "hspmd.ndarray" || type == "ndarray") 
    return ArgType::ND_ARRAY;
  if (type == "List[hspmd.NDArray]" || type == "List[NDArray]" || 
      type == "List[hspmd.ndarray]" || type == "List[ndarray]" || 
      type == "NDArrayList") 
    return ArgType::ND_ARRAY_LIST;
  if (type == "hspmd.Tensor" || type == "Tensor" || 
      type == "hspmd.tensor" || type == "tensor") 
    return ArgType::TENSOR;
  if (type == "List[hspmd.Tensor]" || type == "List[Tensor]" ||
      type == "List[hspmd.tensor]" || type == "List[tensor]" || 
      type == "TensorList") 
    return ArgType::TENSOR_LIST;
  if (type == "hspmd.Operator" || type == "Operator" || 
      type == "hspmd.operator" || type == "operator")
      return ArgType::OPERATOR;
  if (type == "List[hspmd.Operator]" || type == "List[Operator]" ||
      type == "List[hspmd.operator]" || type == "List[operator]" || 
  ...
  if (type == "hspmd.DistributedStates" || type == "DistributedStates")
    return ArgType::DISTRIBUTED_STATES;
  if (type == "List[hspmd.DistributedStates]" || type == "DistributedStatesList" ||
      type == "List[DistributedStates]")
    return ArgType::DISTRIBUTED_STATES_LIST; 
  if (type == "List[hspmd.DistributedStatesUnion]" || type == "List[DistributedStatesUnion]" ||
      type == "DistributedStatesHierarchy")
    return ArgType::DS_HIERARCHY;
  if (type == "hspmd.IntSymbol" || type == "IntSymbol")
    return ArgType::INT_SYMBOL;
  if (type == "List[hspmd.IntSymbol]" || type == "List[IntSymbol]" || type == "SyShape")
    return ArgType::SYMBOLIC_SHAPE;
  if (type == "List[List[hspmd.IntSymbol]]" || type == "List[List[IntSymbol]]" || type == "List[SyShape]")
    return ArgType::SYMBOLIC_SHAPE_LIST;
  if (type == "hspmd.Initializer" || type == "Initializer")
