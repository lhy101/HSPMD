...
PyObject* CommGroup_Init(PyObject*, PyObject* args, PyObject* kwargs) {
  ...
  return PyDevice_New(hspmd::impl::comm::SetUpDeviceMappingAndAssignLocalDeviceOnce({{kCUDA, device_num}}, device_idxs, server_address));
  HT_PY_FUNC_END
}

PyObject* CommGroup_GetLocalDevice(PyObject*, PyObject* args, PyObject* kwargs) {
  HT_PY_FUNC_BEGIN
  return PyDevice_New(hspmd::impl::comm::GetLocalDevice());
  HT_PY_FUNC_END
}

PyObject* CommGroup_GetGlobalDeviceGroup(PyObject*, PyObject* args, PyObject* kwargs) {
  HT_PY_FUNC_BEGIN
  return PyDeviceGroup_New(hspmd::impl::comm::GetGlobalDeviceGroup());
  HT_PY_FUNC_END
}

PyObject* CommGroup_GlobalCommBarrier_RPC(PyObject*, PyObject* args, PyObject* kwargs) {
  HT_PY_FUNC_BEGIN
  std::vector<int> all_ranks(hspmd::impl::comm::GetWorldSize());
  std::iota(all_ranks.begin(), all_ranks.end(), 0);
  hspmd::impl::comm::GetLocalClient()->Barrier(0, all_ranks);
  Py_RETURN_NONE;
  HT_PY_FUNC_END
}
...
PyObject* CommGroup_GlobalCommBarrier_MPI(PyObject*, PyObject* args, PyObject* kwargs) {
  HT_PY_FUNC_BEGIN
  auto& comm_group = hspmd::impl::comm::MPICommunicationGroup::GetOrCreateWorldwide();
...
std::vector<PyMethodDef> InitCommGroupPyClassMethodDefs() {
  ...
  AddPyMethodDefs(ret, hspmd::graph::get_registered_tensor_class_methods());
  return ret;
}
