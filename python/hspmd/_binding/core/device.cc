...
PyObject* PyDevice_reduce(PyDevice* self) {
  ...
  py::object hspmd_module = py::module::import("hspmd");
  py::object hspmd_device = hspmd_module.attr("device");
  PyTuple_SET_ITEM(ret, 0, hspmd_device.release().ptr());
...
PyObject* PyDeviceGroup_reduce(PyDeviceGroup* self) {
  ...
  py::object hspmd_module = py::module::import("hspmd");
  py::object hspmd_device_group = hspmd_module.attr("DeviceGroup");
  PyTuple_SET_ITEM(ret, 0, hspmd_device_group.release().ptr());
...
ContextManager<hspmd::graph::DeviceGroupHierarchy>& get_dg_hierarchy_ctx() {
  static ContextManager<hspmd::graph::DeviceGroupHierarchy> dg_hierarchy_ctx;
