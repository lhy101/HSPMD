...
#include "hspmd/_binding/utils/context_manager.h"
...
inline hspmd::graph::DeviceGroupHierarchy DeviceGroupHierarchy_FromPyObject(PyObject* obj) {
  bool is_tuple = PyTuple_Check(obj);
  size_t size = is_tuple ? PyTuple_GET_SIZE(obj) : PyList_GET_SIZE(obj);
  hspmd::graph::DeviceGroupHierarchy ret;
  for (size_t i = 0; i < size; i++) {
    auto* item = is_tuple ? PyTuple_GET_ITEM(obj, i) : PyList_GET_ITEM(obj, i);
    ret.add(hspmd::graph::DeviceGroupUnion(DeviceGroupList_FromPyObject(item)));
  }
  return ret;
}
