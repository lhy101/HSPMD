...
std::vector<PyMethodDef> InitNDArrayPyMethodDefs() {
  ...
  AddPyMethodDefs(ret, hspmd::impl::get_registered_ndarray_methods());
  return ret;
}
...
std::vector<PyMethodDef> InitNDArrayPyClassMethodDefs() {
  ...
  AddPyMethodDefs(ret, hspmd::impl::get_registered_ndarray_class_methods());
  return ret;
}
