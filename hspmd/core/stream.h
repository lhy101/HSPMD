...
namespace std {
template <>
struct hash<hspmd::Stream> {
  std::size_t operator()(const hspmd::Stream& stream) const noexcept {
    // Following boost::hash_combine
    auto hash = std::hash<hspmd::StreamIndex>()(stream.stream_index());
    hash ^= (std::hash<hspmd::Device>()(stream.device()) + 0x9e3779b9 +
             (hash << 6) + (hash >> 2));
    return hash;
  }
};

inline std::string to_string(const hspmd::Stream& stream) {
