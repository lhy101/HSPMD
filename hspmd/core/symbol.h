...
#include "hspmd/utils/shared_ptr_wrapper.h"
...
namespace std {
  template<>
  struct hash<hspmd::IntSymbol> {
    size_t operator()(const hspmd::IntSymbol& s) const noexcept {
      return hash<std::shared_ptr<hspmd::SymbolDef<int64_t>>>()(s.get_ptr());
    }
  };
} // namespace std