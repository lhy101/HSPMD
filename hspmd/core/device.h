...
class Device {
 public:
  ...
  static inline std::string GetLocalHostname() {
    char* env = std::getenv("HSPMD_LOCAL_HOSTNAME");
    if (env == nullptr) {
      return "";
    } else {
      std::string ret(env);
...
template <>
struct hash<hspmd::Device> {
  std::size_t operator()(const hspmd::Device& device) const noexcept {
...
template <>
struct hash<std::pair<hspmd::Device, hspmd::Device>> {
  std::size_t operator()(const std::pair<hspmd::Device, hspmd::Device>& device_pair) const noexcept {
    // devices in pair need to sort
    auto hash_op = std::hash<hspmd::Device>();
    size_t seed = 2;
...
template <>
struct hash<hspmd::DeviceGroup> {
  std::size_t operator()(const hspmd::DeviceGroup& group) const noexcept {
...
inline std::string to_string(const hspmd::Device& device) {
  std::ostringstream os;
  os << device;
  return os.str();
}

inline std::string to_string(const hspmd::DeviceGroup& group) {
  std::ostringstream os;
  os << group;
  return os.str();
}
