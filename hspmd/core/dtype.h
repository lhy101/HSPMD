...
namespace std {
template <>
struct hash<hspmd::DataType> {
  std::size_t operator()(hspmd::DataType data_type) const noexcept {
    return std::hash<int>()(static_cast<int>(data_type));
  }
};

inline std::string to_string(hspmd::DataType data_type) {
  return hspmd::DataType2Str(data_type);
}
} // namespace std
