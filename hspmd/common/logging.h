...
#include <cstdint>
#include <iomanip>
#include "hspmd/common/collection_streaming.h"
 ...
 public:
  ...
  : _os(os), _verbose(verbose), _file(file), _line(line) {
    static_assert(std::is_base_of<std::exception, Exception>::value,
                  "Exception must be derived from hspmd_exception");
    if (_verbose) {
      auto now = std::chrono::system_clock::now();
      auto time = std::chrono::system_clock::to_time_t(now);
...
#define __HT_LOG_TRACE                                                         \
  hspmd::logging::Logger(__FILENAME__, __LINE__,                                \
                        hspmd::logging::LOG_LEVEL::TRACE, std::cout)
#define __HT_LOG_DEBUG                                                         \
  hspmd::logging::Logger(__FILENAME__, __LINE__,                                \
                        hspmd::logging::LOG_LEVEL::DEBUG, std::cout)
#define __HT_LOG_INFO                                                          \
  hspmd::logging::Logger(__FILENAME__, __LINE__,                                \
                        hspmd::logging::LOG_LEVEL::INFO, std::cout)
#define __HT_LOG_WARN                                                          \
  hspmd::logging::Logger(__FILENAME__, __LINE__,                                \
                        hspmd::logging::LOG_LEVEL::WARN, std::cout)
#define __HT_LOG_ERROR                                                         \
  hspmd::logging::Logger(__FILENAME__, __LINE__,                                \
                        hspmd::logging::LOG_LEVEL::ERROR, std::cerr)

#define HT_LOG(severity)                                                       \
  if (log_level_enabled(hspmd::logging::LOG_LEVEL::severity))                   \
  __HT_LOG_##severity.stream()
#define HT_LOG_IF(severity, cond)                                              \
  if (log_level_enabled(hspmd::logging::LOG_LEVEL::severity) && (cond))         \
  __HT_LOG_##severity.stream()
...
#define __HT_FATAL_VERBOSE(Exception)                                          \
  hspmd::logging::FatalLogger<Exception>(__FILENAME__, __LINE__, true).stream()
#define HT_FATAL_VERBOSE __HT_FATAL_VERBOSE(hspmd::hspmd_exception)
#define __HT_FATAL_SILENT(Exception)                                           \
  hspmd::logging::FatalLogger<Exception>(__FILENAME__, __LINE__, false).stream()
#define HT_FATAL_SILENT __HT_FATAL_SILENT(hspmd::hspmd_exception)
