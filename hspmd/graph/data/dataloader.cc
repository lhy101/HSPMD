...
void Dataloader::init_states() {
  ...
  inst_ctx.start[0] = std::make_unique<hspmd::impl::CPUEvent>();
  inst_ctx.stop[0] = std::make_unique<hspmd::impl::CPUEvent>();
  ...
  hspmd::impl::CPUStream cpu_stream(instantiation_ctx().stream());
  if (_shuffle) {
