...
void OpInterface::DoDeduceStates(const TensorList& inputs, TensorList& outputs, 
  ...
  HT_LOG_TRACE << hspmd::impl::comm::GetLocalDevice() << " " << op_meta.name << ": default copy states from inputs";
...
void OpInterface::DoDeduceHeterProp(const std::vector<int32_t>& inputs_hetero_dim,
  ...
  HT_LOG_TRACE << hspmd::impl::comm::GetLocalDevice() << " " << op_meta.name << ": default copy states hetero dim from inputs";
  ...
  if (output_size > 0) {
    ...
    // 在使用memory plan后大部分output的allocation也会设置好
      ...
      for (size_t i = 0; i < output_size; i++) {
        ...
        // HT_LOG_INFO << hspmd::impl::comm::GetLocalDevice() << ": get runtime shape for " << op->output(i);
        const auto& output_shape = runtime_ctx.get_runtime_shape(output_id);
        HT_LOG_TRACE << hspmd::impl::comm::GetLocalDevice() << ": exec op " << op
        ...
        // alloc on-the-fly
          ...
          // mempool debug use
          /*
          HT_LOG_INFO << hspmd::impl::comm::GetLocalDevice() << ": exec op " << op
...
NDArray OpInterface::DoAllocOutput(Operator& op, const NDArrayList& inputs,
  ...
  // 在使用memory plan后大部分output的allocation也会设置好
    ...
    // HT_LOG_INFO << hspmd::impl::comm::GetLocalDevice() << ": get runtime shape for " << op->output(idx);
    const auto& output_shape = runtime_ctx.get_runtime_shape(output_id);
    HT_LOG_TRACE << hspmd::impl::comm::GetLocalDevice() << ": exec op " << op
...
void OpDef::BlockOrSyncInput(Tensor& input, RuntimeContext& runtime_ctx, size_t micro_batch_id) {
  ...
  if (input_placement.is_undetermined() || input_placement != current_placement) {
    ...
    /*
    HT_LOG_DEBUG << hspmd::impl::comm::GetLocalDevice() << ": op " << name() 
