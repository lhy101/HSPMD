...
#include "hspmd/graph/subgraph.h"
...
NDArray& ExecutableGraph::AllocVariableDataInner(const Tensor& tensor,
  ...
  if (_preserved_data.find(tensor->id()) != _preserved_data.end()) {
    // HT_LOG_DEBUG << hspmd::impl::comm::GetLocalDevice() << ": exec variable " << tensor << " already has the data, so we directly return it";
    return _preserved_data[tensor->id()];
  }
  HT_LOG_DEBUG << hspmd::impl::comm::GetLocalDevice() << ": alloc exec variable " << tensor;
  ...
  // 其余不在buffer中
    ...
    // 分配的是碎片化的显存
    // mempool debug use
    HT_LOG_TRACE << hspmd::impl::comm::GetLocalDevice() << ": on-the-fly alloc variable " << tensor
...
OpHandlerStatus ExecutableGraph::PostOpHandler(Operator& op, Tensor2NDArrayMap& tensor2data, size_t micro_batch_id) {
  ...
  if (is_optimizer_update_op(op)) {
    ...
    if (_run_level == RunLevel::GRAD) {
      ...
      auto event = std::make_unique<hspmd::impl::CUDAEvent>(grad_op->placement());
