...
#include "hspmd/graph/switch_exec_graph.h"
...
#include "hspmd/impl/memory/CUDACachingMemoryPool.cuh"
#include "hspmd/core/device.h"
#include "hspmd/core/dtype.h"
#include "hspmd/core/ndarray_meta.h"
#include "hspmd/core/stream.h"
#include "hspmd/common/timing.h"
#include <nccl.h>
#include <iostream>
#include <fstream>
...
static const std::vector<Device> GetFailureDevices() {
  const char* env = std::getenv("HSPMD_FAILURE_DEVICES");
  if (!env) 
    return {};
  ...
  while (std::getline(ss, token, ',')) {
    ...
    if (!token.empty()) {
      failure_devices.emplace_back(hspmd::impl::comm::WorldRankToDevice(std::stoi(token)));
    }
  }
...
static void WarmUpComm(const std::unordered_set<Device>& comm_set, const StreamIndex comm_stream_idx) {
  ...
  std::transform(comm_set.begin(), comm_set.end(), ranks.begin(), 
                 [&](const Device& device) { return hspmd::impl::comm::DeviceToWorldRank(device); });
  std::sort(ranks.begin(), ranks.end());
  HT_LOG_DEBUG << "all-to-all warm up for " << comm_set
    << ", whose ranks after sort = " << ranks;
  auto& comm_group = hspmd::impl::comm::NCCLCommunicationGroup::GetOrCreate(ranks, Stream(local_device, comm_stream_idx));
...
void ParamBuffer::Alloc(const Stream& stream, 
  ...
  TIK(alloc_time);
  auto local_device = hspmd::impl::comm::GetLocalDevice(); 
  hspmd::cuda::CUDADeviceGuard guard(local_device.index());
  ...
  if (use_nccl) {
    ...
    _storage = std::make_shared<NDArrayStorage>(BorrowToMemoryPool(
      local_device, _raw_ptr, _buffer_size, [=](DataPtr data_ptr) {
        TIK(free_time);
        hspmd::cuda::CUDADeviceGuard guard(data_ptr.device.index());
        HT_LOG_DEBUG << local_device << ": " << _name << " param buffer"
    ...
    if (use_caching_mempool) {
      if (!hspmd::impl::AllocAfterFreeFromCUDACache(local_device, _raw_ptr, _buffer_size)) {
      ...
      if (!use_async || stream.is_blocking()) {
        ...
        status = cudaMallocAsync(&_raw_ptr, _buffer_size, hspmd::impl::CUDAStream(stream).cuda_stream());
      }
    ...
    _storage = std::make_shared<NDArrayStorage>(BorrowToMemoryPool(
      local_device, _raw_ptr, _buffer_size, [=](DataPtr data_ptr) {
        ...
        HT_LOG_DEBUG << local_device << ": " << _name << " param buffer"
          << " will free " << (double)_buffer_size / (1024 * 1024) << " MiB"; 
        if (use_caching_mempool) {
          hspmd::impl::FreeFromCUDACache(local_device, data_ptr.ptr);
        } else {
          cudaError_t status;
          if (!use_async || stream.is_blocking()) {
            status = cudaFree(data_ptr.ptr);
          } else {
            status = cudaFreeAsync(data_ptr.ptr, hspmd::impl::CUDAStream(stream).cuda_stream());
          }
          if (status != cudaSuccess) {
...
void ParamSlice::ParamSliceComm(Device2DTListPairMap& send_mapping,
  ...
  for (size_t i = 0; i < needed_len; ++i) {
    ...
    if (already_owned) {
      ...
      HT_LOG_DEBUG_IF(needed_device == hspmd::impl::comm::GetLocalDevice())
      ...
      if (_switcher->_algorithm_level == SWITCH_ALGORITHM_LEVEL::MIN_IDX) {
        size_t min_index = 0;
        int min_value = hspmd::impl::comm::DeviceToWorldRank(_owned_devices[0]);
        for (size_t i = 1; i < _owned_devices.size(); ++i) {
          int current_value = hspmd::impl::comm::DeviceToWorldRank(_owned_devices[i]);
      ...
      HT_LOG_DEBUG_IF(send_device == hspmd::impl::comm::GetLocalDevice())
...
void SwitchExecGraph::MakeAllParamSlices(const Tensor& param, ParamBlock& block, 
  ...
  if (dim == multiple.size()) {
    ...
    if (hspmd::impl::comm::GetLocalDevice() == device) { 
...
void SwitchExecGraph::BufferBatchedIsendIrecvExec(const hspmd::impl::comm::NCCLCommunicationGroup& comm_nccl_group) {
  auto local_device = hspmd::impl::comm::GetLocalDevice();
  NDArrayList send_data_list;
  NDArrayList recv_data_list;
  std::vector<hspmd::impl::comm::CommTask> tasks;
  ...
  for (const auto& kv : _send_buffers) {
    ...
    tasks.push_back(comm_nccl_group->ISend(send_data, hspmd::impl::comm::DeviceToWorldRank(send_to_device)));
  ...
  for (const auto& kv : _recv_buffers) {
    ...
    tasks.push_back(comm_nccl_group->IRecv(recv_data, hspmd::impl::comm::DeviceToWorldRank(recv_from_device)));
...
void SwitchExecGraph::BufferBatchedIsendIrecv(const Operator& op,
                                              const hspmd::impl::comm::NCCLCommunicationGroup& comm_nccl_group,
  ...
  std::transform(comm_deivces.begin(), comm_deivces.end(), ranks.begin(), 
                 [&](const Device& device) { return hspmd::impl::comm::DeviceToWorldRank(device); });
  std::sort(ranks.begin(), ranks.end());
  auto& comm_group = hspmd::impl::comm::NCCLCommunicationGroup::GetOrCreate(ranks, op_stream);
  */
  ...
  for (size_t i = 0; i < input_len; ++i) {
    ...
    */
    ...
    auto event = std::make_unique<hspmd::impl::CUDAEvent>(local_device);
    ...
    event = std::make_unique<hspmd::impl::CUDAEvent>(local_device);
    event->Record(stream);
    _buffer_transfer_events.emplace_back(std::move(event));
...
void SwitchExecGraph::SwitchParams(SWITCH_MODE switch_mode, 
  ...
  auto& comm_nccl_group = hspmd::impl::comm::NCCLCommunicationGroup::GetOrCreate(comm_ranks, Stream(local_device, comm_stream_idx));
  if (_bucket_num == -1) {
  ...
  if (_profile_level <= SWITCH_PROFILE_LEVEL::TIME) {
    ...
    auto& group = hspmd::impl::comm::NCCLCommunicationGroup::GetOrCreate(hspmd::impl::comm::DeviceGroupToWorldRanks(device_group), local_device);
    if (_comm_set.size() >= 2) {
      group->Barrier(true);
    }
    if (_profile_level <= SWITCH_PROFILE_LEVEL::MEMORY) {
    ...
    for (size_t i = 0; i < op->num_outputs(); ++i) {
      ...
      if (it != _comm_results_mapping.end()) {
        ...
        if (switch_mode == SWITCH_MODE::SWITCH_ORIGIN_PARAM 
            || switch_mode == SWITCH_MODE::SWITCH_TRANSFER_PARAM
            || switch_mode == SWITCH_MODE::SWITCH_ORIGIN_PARAM_AND_OPTIMIZER) {
          auto event = std::make_unique<hspmd::impl::CUDAEvent>(op->placement());
  ...
  if (_profile_level <= SWITCH_PROFILE_LEVEL::TIME) {
    ...
    char* switch_log_file = std::getenv("HSPMD_SWITCH_LOG_FILE");
    if (switch_log_file != nullptr && hspmd::impl::comm::GetWorldRank() == 0) {
      std::ofstream file;
  ...
  // memory debug use
  // hspmd::impl::comm::EmptyNCCLCache();
...
void SwitchExecGraph::ProfileRunningDetails() {
  ...
  for (const auto& kv : send_info_mapping) {
    send_info_output << "send " << kv.second.size()
      << " tensor to device " << hspmd::impl::comm::DeviceToWorldRank(kv.first);
    if (_profile_level <= SWITCH_PROFILE_LEVEL::TIME) {
  ...
  for (const auto& kv : recv_info_mapping) {
    recv_info_output << "recv " << kv.second.size()
      << " tensor from device " << hspmd::impl::comm::DeviceToWorldRank(kv.first);
    if (_profile_level <= SWITCH_PROFILE_LEVEL::TIME) {
