...
#include "hspmd/core/ndarray_storage.h"
#include <nccl.h>
#include <ctime>
#include <iostream>
#include <fstream>
...
Operator& ExecutableGraph::MakeOpInner(std::shared_ptr<OpInterface> body,
  ...
  if (op_meta.device_group_hierarchy.size() != 0) {
    ...
    auto inferred = hspmd::impl::comm::GetLocalDevice();
      ...
      for (size_t i = 0; i < op->num_inputs(); i++) {
        ...
        if (!input->is_contiguous()) {
          ...
          if (op_id.has_value() &&
            ...
            HT_LOG_TRACE << hspmd::impl::comm::GetLocalDevice() << ": Make Contiguous op for tensor " << input->name()
      ...
      if ((comm_type & BATCHED_ISEND_IRECV_OP) != 0) {
        ...
        if (_bridge_single_communicator) {
          for (const auto rank : _used_ranks) {
            comm_set.insert(hspmd::impl::comm::WorldRankToDevice(rank));
          }
        }
        ...
        if (data->device() != input->placement() ||
          ...
          if (data->device().is_cpu() && input->placement().is_cuda()) {
            tensor2data[input->id()] = NDArray::to(data, input->placement(), input->dtype(),
                                                   kH2DStream);
            auto event = std::make_unique<hspmd::impl::CUDAEvent>(input->placement());
...
void ExecutableGraph::GetExecEnvs() {
  ...
  if (env != nullptr) {
    if (std::string(env) == "SINGLE_COMMUNICATOR") {
      ...
      hspmd::impl::comm::NCCLCommunicationGroup::GetOrCreate(_used_ranks, Stream(hspmd::impl::comm::GetLocalDevice(), kP2PStream));
    } else {
      HT_RUNTIME_ERROR << "Unknown hspmd p2p setting: " + std::string(env);
    }
  } else {
  ...
  env = std::getenv("HSPMD_BRIDGE");
  if (env != nullptr) {
    if (std::string(env) == "SINGLE_COMMUNICATOR") {
      _bridge_single_communicator = true;
      // 这里直接提前把communicator给create好
      // HT_LOG_INFO << "get or create single communicator for BRIDGE ops";
      hspmd::impl::comm::NCCLCommunicationGroup::GetOrCreate(_used_ranks, Stream(hspmd::impl::comm::GetLocalDevice(), kBridgeStream));
    } else {
      HT_RUNTIME_ERROR << "Unknown hspmd p2p setting: " + std::string(env);
    }
  } else {
  ...
  env = std::getenv("HSPMD_OVERLAP_GRAD_REDUCE");
  ...
  if (env != nullptr) {
    if (std::string(env) == "FIRST_STAGE") {
      ...
      HT_RUNTIME_ERROR << "Unknown hspmd dp overlap setting: " + std::string(env);
    }
  }

  env = std::getenv("HSPMD_SHAPE_MISMATCH");
  if (env != nullptr) {
    if (std::string(env) == "NO_MISMATCH") {
      ...
      HT_RUNTIME_ERROR << "Unknown hspmd shape mismatch setting: " + std::string(env);
    }
  } else {
    // 默认没有shape mismatch
    _shape_mismatch_flag = 0;
  ...
  env = std::getenv("HSPMD_STRAGGLER");
  if (env != nullptr) {
    if (std::string(env) == "ANALYSIS") {
      ...
      HT_RUNTIME_ERROR << "Unknown hspmd straggler level: " + std::string(env);
    }
  } else {
    // 默认不分析straggler
    _straggler_flag = 0;
  ...
  env = std::getenv("HSPMD_STRAGGLER_LOG_FILE");
  if (env != nullptr) {
  ...
  env = std::getenv("HSPMD_MEMORY_PROFILE");
  if (env != nullptr) {
    if (std::string(env) == "MICRO_BATCH") {
      ...
      HT_RUNTIME_ERROR << "Unknown hspmd memory profile level: " + std::string(env);
    }
  } else {
    // 默认不profile
    _memory_profile_level = MEMORY_PROFILE_LEVEL::WARN;
  ...
  env = std::getenv("HSPMD_MEMORY_LOG_FILE");
  if (env != nullptr) {
  ...
  env = std::getenv("HSPMD_PARALLEL_ATTN");
  if (env != nullptr) {
    if (std::string(env) == "ANALYSIS") {
      ...
      HT_RUNTIME_ERROR << "Unknown hspmd parallel attn level: " + std::string(env);
    }
  } else {
    // 默认不分析parallel attn
  ...
  env = std::getenv("HSPMD_PARALLEL_ATTN_LOG_FILE");
  if (env != nullptr) {
  ...
  env = std::getenv("HSPMD_EVENT_TIMING");
  if (env != nullptr) {
    if (std::string(env) == "ON") {
      ...
      HT_RUNTIME_ERROR << "Unknown hspmd event timing level: " + std::string(env);
    }
  }
}
...
NDArrayList ExecutableGraph::Run(const Tensor& loss, const TensorList& fetches, 
  ...
  if (_used_ranks.size() >= 2) {
    auto& comm_group = hspmd::impl::comm::NCCLCommunicationGroup::GetOrCreate(_used_ranks, local_device);
    comm_group->Barrier(true);
  }
  // sync partially
  /*
  std::vector<int> ranks;
  for (const auto& stage : _pipeline_map[hspmd::impl::comm::GetLocalDevice()]) {
    for (const auto& device : stage.devices()) {
      auto rank = hspmd::impl::comm::DeviceToWorldRank(device);
  ...
  if (ranks.size() >= 2) {
    ...
    // hspmd::impl::comm::Barrier(ranks);
    auto& comm_group = hspmd::impl::comm::NCCLCommunicationGroup::GetOrCreate(ranks, local_device);
    comm_group->Barrier(true);
  }
  */
  ...
  /*
  TIK(free_mempool);
  hspmd::impl::ProfileAfterEmptyAllCUDACache(local_device);
  TOK(free_mempool);
  ...
  */
  ...
  auto profiler_optional = hspmd::impl::Profile::get_cur_profile();
  ...
  if (_memory_profile_level == MEMORY_PROFILE_LEVEL::MICRO_BATCH && _memory_log_file_path != "") {
    std::ofstream file;
    std::string suffix = "_" + std::to_string(hspmd::impl::comm::GetWorldRank()) + ".txt";
    ...
    if (_straggler_flag) {
      ...
      if (_straggler_log_file_path != "") {
        if (_straggler_flag == 1) {
          ...
          ofstream_sync file(_straggler_log_file_path + "_" + std::to_string(hspmd::impl::comm::GetWorldRank()) + ".txt", std::ios_base::app);
          if (file.is_open()) {
          ...
          if (hspmd::impl::comm::GetWorldRank() == 0) {
            ofstream_sync file(_straggler_log_file_path, std::ios_base::app);
            if (file.is_open()) {
              file << "total run time: " << COST_MSEC(crucial_run) << " ms, "
                << "attn fwd time: " << attn_fwd_time << " ms, "
