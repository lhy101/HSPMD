#include "hspmd/graph/define_and_run_graph.h"
...
void DefineAndRunGraph::DeducePipeline(size_t compute_strategy_id, int32_t pipeline_num) {
  ...
  auto it = device_to_pipeline_idx_map.find(hspmd::impl::comm::GetLocalDevice());
...
void DefineAndRunGraph::DeduceShapePlan(ExecGraphPlan& exec_graph_plan,
  ...
  auto local_device = hspmd::impl::comm::GetLocalDevice(); // debug use
...
void DefineAndRunGraph::Instantiate(OpRefList&& global_topo,
  ...
  char* env = std::getenv("HSPMD_PARALLEL_CHANGE_TEST");
  if (env != nullptr) {
...
NDArrayList DefineAndRunGraph::Run(const Tensor& loss, const TensorList& fetches,
  ...
  auto local_device = hspmd::impl::comm::GetLocalDevice(); // only for debug use
  ...
  if (exec_graph->NeedRank(hspmd::impl::comm::DeviceToWorldRank(local_device))) {
  ...
  if (!_is_active || _active_exec_plan != next_active_exec_plan) {
    ...
    if (_is_active) {
      ...
      if (old_exec_graph->_run_level == RunLevel::UPDATE || old_exec_graph->_run_level == RunLevel::COMPUTE_ONLY) {
        ...
        // hspmd::impl::comm::EmptyNCCLCache(); // TODO: this may cause "NCCL call ncclCommInitRank(&_comm, _size, _unique_id, _rank) failed: unhandled system error"
      ...
      */
      if (need_hot_switch) {
        ...
        // 目前已修改成async版本
        // 如果要改成非async的
        // 更改环境变量HSPMD_SWITCH_PROFILE低于TIME即可
        ...
        // 按buckets的顺序进行switch
          ...
          for (int i = 0; i < static_cast<int>(DataType::NUM_DATA_TYPES); i++) {
            ...
            if (_param_and_opt_var_bucket_switcher_pool[key][dtype].empty()) {
              ...
              const auto& global_device_group = hspmd::impl::comm::GetGlobalDeviceGroup();
          ...
          auto& global_mpi_group = hspmd::impl::comm::MPICommunicationGroup::GetOrCreateWorldwide();
  ...
  // hspmd::impl::ProfileAfterEmptyAllCUDACache(local_device); // 验证mempool是否能释放干净
  // workaround: hydraulis need to empty the cuda cache in every run
  const char* pre_allocate_str = std::getenv("HSPMD_PRE_ALLOCATE_SIZE_MB");
