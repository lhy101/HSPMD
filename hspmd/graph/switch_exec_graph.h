...
class ParamBuffer {
  ...
  public:
    ...
    NDArray AsNDArray() {
      ...
      auto meta = NDArrayMeta().set_dtype(_dtype)
                               .set_device(hspmd::impl::comm::GetLocalDevice())
  ...
  protected:
    ...
    std::shared_ptr<NDArrayStorage> _storage; // high-level hspmd mempool api
...
class SwitchExecGraph {
  ...
  public:
    ...
    SwitchExecGraph(DefineAndRunGraph* define_graph, 
      ...
      // 环境变量
      char* algorithm_env = std::getenv("HSPMD_SWITCH_ALGORITHM");
      ...
      char* profile_env = std::getenv("HSPMD_SWITCH_PROFILE");
      ...
      char* buffer_env = std::getenv("HSPMD_SWITCH_BUFFER");
  ...
  protected:
    ...
    void BufferBatchedIsendIrecvExec(const hspmd::impl::comm::NCCLCommunicationGroup& comm_nccl_group);
  ...
  protected:
    ...
    std::vector<std::unique_ptr<hspmd::impl::CUDAEvent>> _buffer_transfer_events; // 记录将原先的param构成一长条buffer的events
