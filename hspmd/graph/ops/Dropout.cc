...
#include "hspmd/impl/random/CPURandomState.h"
...
void DropoutOpImpl::DoCompute(Operator& op, const NDArrayList& inputs,
  ...
  uint64_t seed = hspmd::impl::GenNextRandomSeed();
  ...
  HT_DISPATCH_KERNEL_CUDA_ONLY(op->instantiation_ctx().placement.type(), type(),
                               hspmd::impl::Dropout, inputs.at(0), 1 - keep_prob(),
...
void DropoutGradientOpImpl::DoCompute(Operator& op, const NDArrayList& inputs,
  ...
  HT_DISPATCH_KERNEL_CUDA_ONLY(
    op->instantiation_ctx().placement.type(), type(), hspmd::impl::DropoutGradient, inputs.at(0),
