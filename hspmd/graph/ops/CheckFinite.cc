...
void CheckFiniteOpImpl::DoCompute(Operator& op, 
  ...
  HT_DISPATCH_KERNEL_CUDA_ONLY(op->instantiation_ctx().placement.type(), type(), hspmd::impl::CheckFinite,
...
void CheckNumericOpImpl::DoCompute(Operator& op, 
                                  const NDArrayList& inputs, NDArrayList& outputs,
                                  RuntimeContext& ctx) const {
  HT_DISPATCH_KERNEL_CUDA_ONLY(op->instantiation_ctx().placement.type(), type(), hspmd::impl::CheckNumeric,
                               inputs.at(0), outputs.at(0), op->instantiation_ctx().stream());
}
