...
void VocabParallelCrossEntropyOpImpl::DoCompute(
  ...
  if (op->input(0)->get_local_distributed_states().get_dim(1) == 1) {
    ...
    auto ranks = hspmd::impl::comm::DeviceGroupToWorldRanks(_comm_group);
    hspmd::impl::comm::NCCLCommunicationGroup::GetOrCreate(ranks, op->instantiation_ctx().stream()); // do allreduce 3 times  
    // HT_LOG_INFO << hspmd::impl::comm::GetLocalDevice() << ": VocabParallelCrossEntropyOp: comm_group: " << _comm_group;
    ...
    HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(), type(), // partial -> dup, inplace
                                    hspmd::impl::AllReduce, reduce_max_partial,
    ...
    HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(), type(), // partial -> dup, inplace
                                    hspmd::impl::AllReduce, sum_exp_logits_partial,
    ...
    HT_DISPATCH_KERNEL_CUDA_ONLY(op->instantiation_ctx().placement.type(), type(),
                                hspmd::impl::VocabParallelCrossEntropy, vocab_parallel_logits,
    ...
    HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(), type(), // partial -> dup, inplace
                                    hspmd::impl::AllReduce, predict_logits_partial,
...
void VocabParallelCrossEntropyGradientOpImpl::DoCompute(
  ...
  if (reduction() == kMEAN) {
    HT_DISPATCH_KERNEL_CPU_AND_CUDA(
      op->instantiation_ctx().placement.type(), type(), hspmd::impl::BroadcastShapeMul, grad_output,
      1.0f / broadcasted->numel(), broadcasted, HTAxes(), op->instantiation_ctx().stream());
  } else if (reduction() == kSUM) {
    HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(), type(),
                                    hspmd::impl::BroadcastShape, grad_output,
  ...
  if (op->input(0)->get_local_distributed_states().get_dim(1) == 1) {
    // no tp vocab parallel, just pure dp
    HT_DISPATCH_KERNEL_CPU_AND_CUDA(
      op->instantiation_ctx().placement.type(), type(), hspmd::impl::SoftmaxCrossEntropySparseGradient,
    ...
    HT_DISPATCH_KERNEL_CUDA_ONLY(op->instantiation_ctx().placement.type(), type(),
                                hspmd::impl::VocabParallelCrossEntropyGradient, softmax,
                                labels, vocab_start_index, vocab_end_index, ignored_index(), 
