...
#include "hspmd/graph/executable_graph.h"
#include "hspmd/graph/profiler.h"
...
void AttnInfo::generate_valid_cu_seqlens_and_indices() {
  ...
  CudaMemcpyAsync(_valid_cu_seqlens_q->raw_data_ptr(), valid_cu_seqlens_q_vec.data(), vec_size, cudaMemcpyHostToDevice, hspmd::impl::CUDAStream(Stream(_local_device, kH2DStream)));
  CudaMemcpyAsync(_valid_cu_seqlens_k->raw_data_ptr(), valid_cu_seqlens_k_vec.data(), vec_size, cudaMemcpyHostToDevice, hspmd::impl::CUDAStream(Stream(_local_device, kH2DStream)));
}

AttnCommRing::AttnCommRing(const Operator& op, const hspmd::impl::comm::NCCLCommunicationGroup& nccl_comm_group, StreamIndex stream_idx, 
  ...
  _local_device = hspmd::impl::comm::GetLocalDevice();
  ...
  if (dynamic_cast<ExecutableGraph&>(op->graph())._parallel_attn_flag > 0) {
    ...
    for (size_t i = 0; i < _ring_size; i++) {
      _comm_profile_start_event_list.emplace_back(std::make_shared<hspmd::impl::CUDAEvent>(_local_device));
      _comm_profile_end_event_list.emplace_back(std::make_shared<hspmd::impl::CUDAEvent>(_local_device));
      _attn_profile_start_event_list.emplace_back(std::make_shared<hspmd::impl::CUDAEvent>(_local_device));
      _attn_profile_end_event_list.emplace_back(std::make_shared<hspmd::impl::CUDAEvent>(_local_device));
      _corr_profile_start_event_list.emplace_back(std::make_shared<hspmd::impl::CUDAEvent>(_local_device));
      _corr_profile_end_event_list.emplace_back(std::make_shared<hspmd::impl::CUDAEvent>(_local_device));
      _grad_profile_start_event_list.emplace_back(std::make_shared<hspmd::impl::CUDAEvent>(_local_device));
      _grad_profile_end_event_list.emplace_back(std::make_shared<hspmd::impl::CUDAEvent>(_local_device));
    }
  }  
  auto env = std::getenv("HSPMD_PARALLEL_ATTN_SPLIT_PATTERN");
  if (env != nullptr) {
    if (std::string(env) == "NORMAL") {
      ...
      HT_RUNTIME_ERROR << "Unknown hspmd parallel attn split pattern: " + std::string(env);
    }
  } else {
    // 默认使用NORMAL
    _attn_split_pattern = AttnSplitPattern::NORMAL;
...
void AttnCommRing::GenerateAttnInfo() {
  ...
  if (_packing) {
    ...
    auto event = std::make_unique<hspmd::impl::CUDAEvent>(_local_device);
    ...
    hspmd::cuda::CUDADeviceGuard guard(_local_device.index());
    ...
    for (size_t i = 0; i < _ring_size; i++) {
      ...
      CudaMemcpyAsync(cu_seqlens_q_vecs[i].data(), q_dev_ptr + vec_size * i, vec_size, cudaMemcpyDeviceToHost, hspmd::impl::CUDAStream(Stream(_local_device, kD2HStream)));
      CudaMemcpyAsync(cu_seqlens_k_vecs[i].data(), k_dev_ptr + vec_size * i, vec_size, cudaMemcpyDeviceToHost, hspmd::impl::CUDAStream(Stream(_local_device, kD2HStream)));
    }
    ...
    */
    CudaStreamSynchronize(hspmd::impl::CUDAStream(Stream(_local_device, kD2HStream)));
  }
  // q所在的rank是i
  for (size_t i = 0; i < _ring_size; i++) {
    // k和v所在的rank是j
...
void AttnCommRing::ExecFlashAttn(int64_t q_idx, int64_t kv_idx, 
  ...
  if (!_packing) {
    if (!is_bwd) {
      ...
      HT_DISPATCH_KERNEL_CUDA_ONLY(DeviceType::CUDA, "FlashAttn", hspmd::impl::FlashAttn,
      ...
      HT_DISPATCH_KERNEL_CUDA_ONLY(DeviceType::CUDA, "FlashAttnGradient", hspmd::impl::FlashAttnGradient,
  ...
  // 且需要把batch维度给去掉 
    ...
    if (!is_bwd) {
      ...
      HT_DISPATCH_KERNEL_CUDA_ONLY(DeviceType::CUDA, "FlashAttnVarlen", hspmd::impl::FlashAttnVarlen, 
      ...
      HT_LOG_TRACE << _local_device << ": before attn " << q_idx << "-th q slice sum is " << NDArray::sum(q_slice) << " and " << kv_idx << "-th k(v) slice sum is " << NDArray::sum(k_slice)
        << ", valid cu_seqlens_q is " << attn_info->get_valid_cu_seqlens_q() << ", valid cu_seqlens_k is " << attn_info->get_valid_cu_seqlens_k();
      HT_DISPATCH_KERNEL_CUDA_ONLY(DeviceType::CUDA, "FlashAttnVarlenGradient", hspmd::impl::FlashAttnVarlenGradient, 
...
void AttnCommRing::ExecComm(const NDArray& send_data, const NDArray& recv_data,
  ...
  // auto start_event = std::make_shared<hspmd::impl::CUDAEvent>(_local_device);
  ...
  std::vector<hspmd::impl::comm::CommTask> ring_send_recv_tasks;
  ...
  for (size_t i = 0; i < dst_split_num; i++) {
    ...
    ring_send_recv_tasks.push_back(_nccl_comm_group->ISend(send_data_slice, hspmd::impl::comm::DeviceToWorldRank(dst_devices[i])));
  ...
  for (size_t i = 0; i < src_split_num; i++) {
    ...
    ring_send_recv_tasks.push_back(_nccl_comm_group->IRecv(recv_data_slice, hspmd::impl::comm::DeviceToWorldRank(src_devices[i])));
  ...
  // auto end_event = std::make_shared<hspmd::impl::CUDAEvent>(_local_device);
  ...
  for (size_t i = 0; i < src_split_num; i++) {
    HT_DISPATCH_KERNEL_CUDA_ONLY(DeviceType::CUDA, "AttnCommConcatRecvSlices",
                                 hspmd::impl::Concatenate, recv_data_slices[i],
...
void AttnCommRing::Run(bool is_bwd) {
  ...
  auto comp_stream = Stream(_local_device, _stream_idx);
  auto tmp_event = std::make_unique<hspmd::impl::CUDAEvent>(_local_device);
  ...
  */
  ...
  if (tp_group_list.size() >= 2) {
    ...
    */
    ...
    auto local_device = hspmd::impl::comm::GetLocalDevice();
    auto& nccl_comm_group = hspmd::impl::comm::NCCLCommunicationGroup::GetOrCreate(used_ranks, local_device);
  ...
  // 不开cp
    ...
    if (!_packing) {
      ...
      HT_DISPATCH_KERNEL_CUDA_ONLY(op->instantiation_ctx().placement.type(), type(), hspmd::impl::FlashAttn,
      ...
      HT_DISPATCH_KERNEL_CUDA_ONLY(op->instantiation_ctx().placement.type(), type(), hspmd::impl::FlashAttnVarlen, 
...
void ParallelAttentionGradientOpImpl::DoCompute(Operator& op, const NDArrayList& inputs,
  ...
  */
  ...
  if (tp_group_list.size() >= 2) {
    ...
    */
    auto used_ranks = dynamic_cast<ExecutableGraph&>(op->graph()).GetUsedRanks();
    auto& nccl_comm_group = hspmd::impl::comm::NCCLCommunicationGroup::GetOrCreate(used_ranks, hspmd::impl::comm::GetLocalDevice());
    auto attn_comm_ring = AttnCommRing(op,
                                       nccl_comm_group, 
                                       stream_idx,
                                       ring_idx,
  ...
  // 不开cp
    ...
    if (!_packing) {
      HT_DISPATCH_KERNEL_CUDA_ONLY(op->instantiation_ctx().placement.type(), type(),
                                   hspmd::impl::FlashAttnGradient, reshaped_grad_output,
      ...
      */
      ...
      HT_DISPATCH_KERNEL_CUDA_ONLY(op->instantiation_ctx().placement.type(), type(),
                                   hspmd::impl::FlashAttnVarlenGradient, reshaped_grad_output,
