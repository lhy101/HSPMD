...
#include "hspmd/graph/ops/Views.h"
...
class SliceOpImpl final : public ViewsOpImpl {
 public:
  ...
  : ViewsOpImpl(quote(SliceOp)),
    ...
    _symbolic(true) {
    HT_LOG_TRACE << hspmd::impl::comm::GetLocalDevice() << " splice op begin_pos = " << get_begin_pos();
    HT_LOG_TRACE << hspmd::impl::comm::GetLocalDevice() << " splice op begin_pos = " << get_output_shape();
  }
