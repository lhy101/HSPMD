...
void BatchNormOpImpl::DoCompute(Operator& op, 
  ...
  //                                 hspmd::impl::ArraySet, const_cast<NDArray&>(inputs.at(3)), 0,
  //                                 op->instantiation_ctx().stream());

  // HT_DISPATCH_KERNEL_CPU_AND_CUDA(
  //   op->instantiation_ctx().placement.type(), type(), hspmd::impl::ArraySet, 
  ...
  NDArray::arrayset(const_cast<NDArray&>(inputs.at(4)), 1,
                    op->instantiation_ctx().stream_index);
  // HT_DISPATCH_KERNEL_CPU_AND_CUDA(
  //   op->instantiation_ctx().placement.type(), type(), hspmd::impl::BatchNorm, inputs.at(0),
...
void BatchNormGradientOpImpl::DoCompute(Operator& op,
                                        ...
                                        RuntimeContext& ctx) const {
  HT_DISPATCH_KERNEL_CPU_AND_CUDA(
    op->instantiation_ctx().placement.type(), type(), hspmd::impl::BatchNormGradient, inputs.at(0),
