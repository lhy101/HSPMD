...
bool CommOpImpl::DoInstantiate(Operator& op, const Device& placement,
  ...
  if (placement.is_cuda()) {
    for (size_t i = 0; i < HT_MAX_NUM_MICRO_BATCHES; i++) { 
      inst_ctx.start[i] = std::make_unique<hspmd::impl::CUDAEvent>(placement, enable_timing);
      inst_ctx.stop[i] = std::make_unique<hspmd::impl::CUDAEvent>(placement, enable_timing);
    }
  } else {
    for (size_t i = 0; i < HT_MAX_NUM_MICRO_BATCHES; i++) {     
      inst_ctx.start[i] = std::make_unique<hspmd::impl::CPUEvent>(enable_timing);
      inst_ctx.stop[i] = std::make_unique<hspmd::impl::CPUEvent>(enable_timing);
    }
  }
...
NDArrayList AllReduceOpImpl::DoCompute(Operator& op,
  ...
  HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(), type(),
                                  hspmd::impl::AllReduce, inputs.at(0),
...
void P2PSendOpImpl::DoCompute(Operator& op, 
  ...
  // HT_LOG_INFO << "rank " << hspmd::impl::comm::DeviceToWorldRank(op->instantiation_ctx().placement) << " will send to rank " << hspmd::impl::comm::DeviceToWorldRank(_dst_group.get(dst_device_index));
  HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(), 
                                  type(), hspmd::impl::P2PSend, input,
...
void P2PRecvOpImpl::DoCompute(Operator& op, 
  ...
  // HT_LOG_INFO << "rank " << hspmd::impl::comm::DeviceToWorldRank(op->instantiation_ctx().placement) << " will recv from rank " << hspmd::impl::comm::DeviceToWorldRank(_src_group.get(src_device_index));
  HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(),
                                  type(), hspmd::impl::P2PRecv, outputs.at(0),
...
void BatchedISendIRecvOpImpl::DoCompute(Operator& op, 
  ...
  HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(), type(), 
                                  hspmd::impl::BatchedISendIRecv, contig_inputs, _dst_devices, outputs, 
...
void AllGatherOpImpl::DoCompute(Operator& op, 
  ...
  HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(), type(),
                                  hspmd::impl::AllGather, inputs.at(0), outputs.at(0), 
...
NDArrayList ReduceScatterOpImpl::DoCompute(Operator& op,
  ...
  HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(), type(),
                                  hspmd::impl::ReduceScatter, inputs.at(0), outputs.at(0), 
...
NDArrayList SplitAllGatherOpImpl::DoCompute(Operator& op,
  ...
  for (size_t i = 0; i < _comm_groups_list.size(); i++) {
    ...
    for (const auto& comm_group : comm_groups) {
      HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(), type(),
                                      hspmd::impl::AllGather, split_inputs.at(i),
...
NDArrayList SplitAllReduceOpImpl::DoCompute(Operator& op,
  ...
  for (size_t i = 0; i < _comm_groups_list.size(); i++) {
    ...
    for (const auto& comm_group : comm_groups) {
      HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(), type(),
                                      hspmd::impl::AllReduce, split_inputs.at(i),
...
NDArrayList SplitReduceScatterOpImpl::DoCompute(Operator& op,
  ...
  for (size_t i = 0; i < _comm_groups_list.size(); i++) {
    ...
    for (const auto& comm_group : comm_groups) {
      HT_DISPATCH_KERNEL_CPU_AND_CUDA(op->instantiation_ctx().placement.type(), type(),
                                      hspmd::impl::ReduceScatter, split_inputs.at(i),
