...
class AttnStorage {
 public: 
  ...
  : _storage(storage) {
    _comm_event = std::make_shared<hspmd::impl::CUDAEvent>(storage->device());
    _attn_event = std::make_shared<hspmd::impl::CUDAEvent>(storage->device());
    _grad_event = std::make_shared<hspmd::impl::CUDAEvent>(storage->device());
  }
 ...
 protected:
  ...
  bool _is_comm{false};
  std::shared_ptr<hspmd::impl::CUDAEvent> _comm_event;
  bool _is_attn{false};
  std::shared_ptr<hspmd::impl::CUDAEvent> _attn_event;
  bool _is_grad{false};
  std::shared_ptr<hspmd::impl::CUDAEvent> _grad_event;
};
...
class AttnCommRing {
 public:
  AttnCommRing(const Operator& op, const hspmd::impl::comm::NCCLCommunicationGroup& nccl_comm_group, StreamIndex stream_idx, 
 ...
 protected:
  ...
  hspmd::impl::comm::NCCLCommunicationGroup _nccl_comm_group;
  int64_t _ring_idx;
  int64_t _ring_size;
  ...
  std::vector<std::shared_ptr<hspmd::impl::CUDAEvent>> _comm_profile_start_event_list, _comm_profile_end_event_list, _attn_profile_start_event_list, _attn_profile_end_event_list, _corr_profile_start_event_list, _corr_profile_end_event_list, _grad_profile_start_event_list, _grad_profile_end_event_list;
};
