...
struct OpProfilerInfo {
  hspmd::graph::OpType type;
  hspmd::graph::OpName name;
...
class Profile {
 public:
  ...
  void sync_op() {
    if (!enabled())
      return;
    if (!_ops.empty()) {
      std::unordered_set<hspmd::Stream> _sync_stream;
      for (auto& op : _ops) {
      ...
      for (auto& op : _ops) {
        HTShapeList inputs_shape;
	      hspmd::graph::Operator::for_each_input_tensor(op, [&](const hspmd::graph::Tensor& input) {
  ...
  std::vector<std::pair<hspmd::graph::OpType, std::pair<double, std::pair<double, int>>>>
  get_optype_view() {
    sync_op();
    std::vector<std::pair<hspmd::graph::OpType, std::pair<double, std::pair<double, int>>>> single_optype_total_time;
    std::map<hspmd::graph::OpType, std::pair<double, int>> single_optype_total_time_unordered;
    ...
    std::sort(single_optype_total_time.begin(), single_optype_total_time.end(),
      [&](std::pair<hspmd::graph::OpType, std::pair<double, std::pair<double, int>>> x, std::pair<hspmd::graph::OpType, std::pair<double, std::pair<double, int>>> y) {
                return x.second.second.first > y.second.second.first; });
    return single_optype_total_time;
  }

  std::vector<std::pair<std::pair<hspmd::graph::OpType, HTShapeList>, std::pair<double, std::pair<double, int>>>>
  get_optype_with_inputs_view() {
    sync_op();

    std::vector<std::pair<std::pair<hspmd::graph::OpType, HTShapeList>, std::pair<double, std::pair<double, int>>>> single_optype_with_inputs_total_time;
    std::map<std::pair<hspmd::graph::OpType, HTShapeList>, std::pair< double, int>> single_optype_with_inputs_total_time_unordered;
    for (auto& record: _op_record) {
    ...
    std::sort(single_optype_with_inputs_total_time.begin(), single_optype_with_inputs_total_time.end(),
      [&](std::pair<std::pair<hspmd::graph::OpType, HTShapeList>, std::pair<double, std::pair<double, int>>> x, std::pair<std::pair<hspmd::graph::OpType, HTShapeList>, std::pair<double, std::pair<double, int>>> y) {
            return x.second.second.first > y.second.second.first; });
    return single_optype_with_inputs_total_time;
  }

  std::vector<std::pair<hspmd::graph::OpType, double>> get_graph_view() {
 ...
 protected:
  ...
  std::vector<hspmd::graph::Operator> _ops;
