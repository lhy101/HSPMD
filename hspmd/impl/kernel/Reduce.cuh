...
template <>
struct OpMathType<hspmd::bfloat16> {
  using type = float;
};
template <>
struct OpMathType<hspmd::float16> {
  using type = float;
};
template <typename T>
...
__device__ void block_x_reduce(arg_t* value, char* shared_memory, op_t ops) {
  ...
  for (int offset = 1; offset < dim_x; offset <<= 1) {
    #pragma unroll
    for (int i = 0; i < output_vec_size; i++) {
      arg_t other = hspmd::cuda::shfl_down_sync(mask, value[i], offset, warpSize);
...
void launch_reduce_kernel(const NDArray& in_arr, NDArray& out_arr, const int64_t* axes,
  ...
  auto reduce_meta_arr = hspmd::cuda::to_int64_ndarray(reduce_meta, device_id);
