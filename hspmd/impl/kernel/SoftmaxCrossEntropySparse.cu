...
__global__ void softmax_cross_entropy_sparse_kernel(const spec_t* pred, const int64_t* label, 
  ...
  for (size_t ptr = start_idx; ptr < end_idx; ptr += stride) {
    ...
    maxval = hspmd::cuda::cuda_max(pred[pred_offset], maxval);
  }
  hspmd::cuda::BlockReduceArgmax(maxval, buffer, wrap_max);
  maxval = wrap_max[0];
  float sum = 0;
  for (size_t ptr = start_idx; ptr < end_idx; ptr += stride) {
    pred_offset = pred_offset_calculator->get(ptr);
    sum += hspmd::cuda::cuda_exp(float(pred[pred_offset] - maxval));
  }
  hspmd::cuda::BlockReduceSum(sum, buffer_f);
  if (threadIdx.x == 0) {
    size_t curid = idx * n_cols + int64_t(label[label_offset]);
    pred_offset = pred_offset_calculator->get(curid);
    loss[loss_offset] = - pred[pred_offset] + maxval + spec_t(hspmd::cuda::cuda_log(sum));
  }
}
...
__global__ void softmax_cross_entropy_sparse_kernel2(const spec_t* pred, const int64_t* label, 
  ...
  for (size_t i = 1; i < n_cols; ++i) {
    ...
    maxval = hspmd::cuda::cuda_max(maxval, pred[pred_offset]);
  }
  ...
  pred_offset = pred_offset_calculator->get(curid);
  // loss[idx] = -(pred[curid] - maxval) + hspmd::cuda::cuda_log(sum);
  loss[loss_offset] = - pred[pred_offset] + maxval + spec_t(hspmd::cuda::cuda_log(sum));
}
...
softmax_cross_entropy_sparse_gradient_kernel(const spec_t* pred, const int64_t* label,
  ...
  for (size_t i = 0; i < n_cols; ++i) {
      ...
      if(i == int64_t(label[label_offset]))
        output[out_offset] = (hspmd::cuda::cuda_exp(pred[pred_offset] - maxval) / spec_t(sum) - 1.0)
                           * grad_loss[grad_loss_offset];
      else
        output[out_offset] = (hspmd::cuda::cuda_exp(pred[pred_offset] - maxval) / spec_t(sum))
                           * grad_loss[grad_loss_offset];
  }
}
