...
void launch_vectorize_broadcast_loop_kernel(const NDArray& inputA, const NDArray& inputB, NDArray& output,
  ...
  auto strideA_arr = hspmd::cuda::to_int64_buffer<ndim>(strideA);       \
  auto strideB_arr = hspmd::cuda::to_int64_buffer<ndim>(strideB);       \
  auto out_stride_arr = hspmd::cuda::to_int64_buffer<ndim>(out_stride);
...
void launch_broadcast_loop_kernel(const NDArray& inputA, const NDArray& inputB, NDArray& output,
  ...
  if (contiguous) {
    ...
    auto broadcast_metadata_arr = hspmd::cuda::to_int64_ndarray(broadcast_metadata, cuda_stream.device_id());
