...
void AsStridedCuda(const NDArray& input, NDArray& output,
  ...
  auto stride_in_arr = hspmd::cuda::to_int64_ndarray(stride, device_id);
  auto stride_out_arr =
    hspmd::cuda::to_int64_ndarray(output->stride(), device_id);
  dim3 blocks, threads;
...
__global__ void asstrided_gradient_kernel(const spec_t* input, spec_t* output,
  ...
  hspmd::cuda::AtomicAdd(&output[index], input[idx]);
}
...
void AsStridedGradientCuda(const NDArray& output, NDArray& input,
  ...
  auto device_id = input->device().index();
  hspmd::cuda::CUDADeviceGuard guard(input->device().index());
  CUDAStream cuda_stream(stream);
  auto stride_in_arr =
    hspmd::cuda::to_int64_ndarray(stride, device_id);
  auto stride_out_arr =
    hspmd::cuda::to_int64_ndarray(output->stride(), device_id);
  dim3 blocks, threads;
  threads.x = MIN(size, HT_DEFAULT_NUM_THREADS_PER_BLOCK);
...
__global__ void view_asstrided_gradient_kernel(const spec_t *input, spec_t *output, size_t size,
  ...
  hspmd::cuda::AtomicAdd(&output[index], input[in_offset]);
}
...
__global__ void count_kernel(spec_t *output, size_t size,
  ...
  hspmd::cuda::AtomicAdd(&output[index], ident);
}
...
void AsStridedGradientCuda(const NDArray& grad_output, NDArray& grad_input,
  ...
  auto out_stride_contig_arr = 
    hspmd::cuda::to_int64_ndarray(out_stride_contig, device_id);
  auto out_stride_arr =
    hspmd::cuda::to_int64_ndarray(out_stride, device_id);
  ...
  if (in_maybe_overlap) {
    ...
    auto in_stride_contig_arr =
      hspmd::cuda::to_int64_ndarray(in_stride_contig, device_id);
    auto in_stride_arr = 
      hspmd::cuda::to_int64_ndarray(in_stride, device_id);
