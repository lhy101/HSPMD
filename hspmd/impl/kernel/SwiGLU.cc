...
#pragma omp parallel for schedule(static)
  ...
  for (size_t idx = 0; idx < size; idx++) {
    auto in_idx = (idx / d) * (2 * d) + (idx % d);
    int64_t i_x_idx = hspmd::impl::get_index(in_idx, ndims, stride, c_shape);
    int64_t i_y_idx = hspmd::impl::get_index(in_idx + d, ndims, stride, c_shape);
...
#pragma omp parallel for schedule(static)
  ...
  for (size_t idx = 0; idx < size; idx++) {
    ...
    int64_t i_x_idx = hspmd::impl::get_index(in_idx, ndims, stride, c_shape);
    int64_t i_y_idx = hspmd::impl::get_index(in_idx + d, ndims, stride, c_shape);
    int64_t ig_x_idx = hspmd::impl::get_index(in_idx, ndims, stride_in, c_shape);
    int64_t ig_y_idx =
      hspmd::impl::get_index(in_idx + d, ndims, stride_out, c_shape);
    int64_t o_idx = hspmd::impl::get_index(idx, ndims, stride_out, c_shape);
