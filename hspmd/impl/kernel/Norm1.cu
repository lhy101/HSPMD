...
__device__ spec_t sgn(spec_t x) {
    if (x == 0.0)
        return 0.0;
    return x / hspmd::cuda::cuda_abs(x);
}
...
__global__ void norm_kernel(const spec_t* input, spec_t* output, size_t size, 
  ...
  for (size_t ptr = start_ptr; ptr < end_ptr; ptr += stride) {
      ...
      sum_thread += hspmd::cuda::cuda_pow(spec_t(hspmd::cuda::cuda_abs(input[in_offset])), spec_t(p));
  }
  hspmd::cuda::BlockReduceSum(sum_thread, shared_sum);
  if (threadIdx.x == 0) {
      auto out_offset = out_offset_calculator->get(output_ptr);
      output[out_offset] = hspmd::cuda::cuda_pow(spec_t(sum_thread), spec_t(1.0 / p));
  }
}
...
__global__ void norm_gradient_kernel(const spec_t *input, const spec_t *norm,
  ...
  if (p == 1) {
      ...
      if (norm_val == 0)
          ...
          output[out_offset] = input_val * hspmd::cuda::cuda_pow(hspmd::cuda::cuda_abs(input_val), spec_t(p - 2)) * grad_val
                        / hspmd::cuda::cuda_pow(norm_val, spec_t(p - 1));
  }
}
