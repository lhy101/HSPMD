...
void SliceCuda(const NDArray& input, NDArray& output, const HTShape& begin_pos,
  ...
  auto pos_arr = hspmd::cuda::to_int64_ndarray(begin_pos, device_id);
  auto i_shape_arr = hspmd::cuda::to_int64_ndarray(input->shape(), device_id);
  auto o_shape_arr = hspmd::cuda::to_int64_ndarray(output->shape(), device_id);
  dim3 blocks, threads;
...
void SliceGradientCuda(const NDArray& output_grad, NDArray& input_grad,
  ...
  auto pos_arr = hspmd::cuda::to_int64_ndarray(begin_pos, device_id);
  auto i_shape_arr =
    hspmd::cuda::to_int64_ndarray(output_grad->shape(), device_id);
  auto o_shape_arr =
    hspmd::cuda::to_int64_ndarray(input_grad->shape(), device_id);
  dim3 blocks, threads;
  threads.x = MIN(size, HT_DEFAULT_NUM_THREADS_PER_BLOCK);
  blocks.x = DIVUP(size, HT_DEFAULT_NUM_THREADS_PER_BLOCK);
  HT_DISPATCH_INTEGER_AND_FLOATING_TYPES(
