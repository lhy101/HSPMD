...
__global__ void instance_norm_kernel(const spec_t* x,  spec_t* y, spec_t* mean,
  ...
  hspmd::cuda::BlockReduceSum(mean_thread, shared_mean);
  hspmd::cuda::BlockReduceSum(var_thread, shared_var);
  if (threadIdx.x == 0) {
...
__global__ void instancenorm_grad_kernel(const spec_t* out_grads,
  ...
  grad_arr[grad_offset] = out_grads[out_grads_offset] / hspmd::cuda::cuda_sqrt(var_arr[var_offset] + eps) +
    ((tmp - dbias[mo_idx]) / (spec_t)last2dim) / 
    hspmd::cuda::cuda_sqrt(var_arr[var_offset] + eps);
}
...
void InstanceNormGradientCuda(const NDArray& out_grads, const NDArray& in_arr,
  ...
  cudnnHandle_t handle = hspmd::impl::GetCudnnHandle(cuda_stream.device_id());
