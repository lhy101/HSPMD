...
void kldivloss_cpu(const spec_t* pred,
  ...
  for (size_t idx = 0; idx < n_rows; ++idx) {
    int64_t pred_idx = hspmd::impl::get_index(idx, ndims, stride_pred, c_shape);
    int64_t label_idx = hspmd::impl::get_index(idx, ndims, stride_label, c_shape);
    int64_t loss_idx = hspmd::impl::get_index(idx, ndims, stride_loss, c_shape);
