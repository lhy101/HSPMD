...
#endif
  for (size_t idx = 0; idx < size; ++idx) {
    int64_t o_idx = hspmd::impl::get_index(idx, ndims, stride, c_shape);
    int64_t og_idx = hspmd::impl::get_index(idx, ndims, stride_out, c_shape);
    int64_t ig_idx = hspmd::impl::get_index(idx, ndims, stride_in, c_shape);
    output[idx] = (1 - input[idx] * input[idx]) * output_grad[idx];
  }
}
    ...
    input->dtype(), spec_t, "TanhCpu", [&]() {
      auto _future = cpu_stream.EnqueueTask(
        [stream, input, output, size]() {
          ...
          auto dnnltype = hspmd::cpu::dtype_to_dnnltype(input->dtype());
