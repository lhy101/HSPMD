...
void RollCuda(const NDArray& input, const HTShape& shift, const HTAxes& axis,
  ...
  auto strides_arr = hspmd::cuda::to_int64_ndarray(strides, device_id);
  auto sizes_arr = hspmd::cuda::to_int64_ndarray(sizes, device_id);
  auto shifts_arr = hspmd::cuda::to_int64_ndarray(shifts, device_id);
  dim3 blocks, threads;
