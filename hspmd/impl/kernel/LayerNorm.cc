...
#include "hspmd/impl/utils/dnnl_utils.h"
...
void LayerNormCpu(const NDArray& in_arr, const NDArray& ln_scale,
  ...
  HT_DISPATCH_INTEGER_AND_FLOATING_TYPES(
    in_arr->dtype(), spec_t, "LayerNormCpu", [&]() {
      ...
      auto dnnltype = hspmd::cpu::dtype_to_dnnltype(in_arr->dtype());
      ...
      if (in_arr->shape() == mean_arr->shape())
        hspmd::cpu::read_from_dnnl_memory(mean_arr->data_ptr<spec_t>(), src_mem);
      else {
      ...
      if (in_arr->shape() == mean_arr->shape())
        hspmd::cpu::read_from_dnnl_memory(var_arr->data_ptr<spec_t>(), src_mem);
      else {
  ...
  HT_DISPATCH_FLOATING_TYPES(
    in_arr->dtype(), spec_t, "LayerNormGradientCpu", [&]() {
      ...
      if (in_arr->shape() == ln_scale->shape())
        hspmd::cpu::read_from_dnnl_memory(grad_bias->data_ptr<spec_t>(), src_mem);
      else {
      ...
      if (in_arr->shape() == ln_scale->shape())
        hspmd::cpu::read_from_dnnl_memory(grad_scale->data_ptr<spec_t>(), src_mem);
      else {
      ...
      if (in_arr->shape() == mean_arr->shape())
        hspmd::cpu::read_from_dnnl_memory(db, src_mem);
      else {
      ...
      if (in_arr->shape() == ln_scale->shape())
        hspmd::cpu::read_from_dnnl_memory(ds, mdst_mem);
      else {
