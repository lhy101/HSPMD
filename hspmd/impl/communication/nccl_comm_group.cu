...
#include "hspmd/utils/task_queue.h"
...
static int32_t get_shape_mismatch_flag() {
  auto& graph = hspmd::graph::Graph::GetGraph(hspmd::graph::Graph::cur_graph_ctx());
  if (graph.type() == hspmd::graph::GraphType::EXECUTABLE) {
    return dynamic_cast<hspmd::graph::ExecutableGraph&>(graph).shape_mismatch_flag();
  }
  return 0;
}
...
: CommunicationGroupDef(world_ranks, stream) {
  ...
  {
    hspmd::cuda::CUDADeviceGuard guard(_stream.device_index());
    {
