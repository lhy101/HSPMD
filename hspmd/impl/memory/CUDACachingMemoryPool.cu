...
#include "hspmd/impl/memory/memory_manager.h"
...
: CUDAMemoryPool(device_id, _make_name(device_id)),
  ...
  if (pre_allocate_size > 0) {
    void* ptr;
    TIK(pre_allocate);
    hspmd::cuda::CUDADeviceGuard guard(device_id);
    ...
    if (err != cudaSuccess) {
      HT_RUNTIME_ERROR << "cudaMalloc failed when tring to pre-allocate " << pre_allocate_size << " byte for the CUDACachingMemoryPool on device " << static_cast<int32_t>(device_id)
        ...
        << ", you may need to adjust HSPMD_PRE_ALLOCATE_SIZE_MB";
    } else {
  ...
  // *只有这种情况会AllocPtr并将新分配的data ptr放入到info中
    ...
    // 清空cache后依然无法分配
      ...
      HT_RUNTIME_ERROR
        ...
        << ", please set environment variable HSPMD_MAX_SPLIT_SIZE_MB smaller"
  ...
  for (auto& kv : _single_stream_free_events) {
    ...
    while (!stream_free_events.empty()) {
      ...
      if (status == cudaErrorNotReady) {
        ...
        __HT_FATAL_SILENT(hspmd::cuda::cuda_error)
...
static size_t ParseMaxSplitSize() {
  const char* max_split_str = std::getenv("HSPMD_MAX_SPLIT_SIZE_MB");
  ...
  if (max_split_str != NULL) {
    try {
      ...
      HT_LOG_WARN
        << "Invalid HSPMD_MAX_SPLIT_SIZE_MB: " << max_split_str << " is set" 
...
static size_t ParseMaxInternalFragmentSize() {
  const char* max_internal_fragment_str = std::getenv("HSPMD_MAX_INTERNAL_FRAGMENT_SIZE_MB");
  ...
  if (max_internal_fragment_str != NULL) {
    try {
      ...
      HT_LOG_WARN
        << "Invalid HSPMD_MAX_INTERNAL_FRAGMENT_SIZE_MB: " << max_internal_fragment_str << " is set" 
        << ", please provide an integer"
        << ", default value will be used in this process.";
    }
  } 
...
static size_t ParsePreAllocateSize() {
  ...
  if (pre_allocate_str != NULL) {
    try {
      ...
      HT_LOG_WARN
        << "Invalid HSPMD_PRE_ALLOCATE_SIZE_MB: " << pre_allocate_str << " is set" 
        << ", please provide an integer"
        << ", default value will be used in this process.";
    }
  } 
