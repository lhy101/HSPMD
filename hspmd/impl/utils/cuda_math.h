...
template <>
__forceinline__ __device__ hspmd::bfloat16 shfl_down_sync<hspmd::bfloat16>(const unsigned mask, hspmd::bfloat16 var, const unsigned int delta, const int width) {
...
template <>
__forceinline__ __device__ hspmd::float16 shfl_down_sync<hspmd::float16>(const unsigned mask, hspmd::float16 var, const unsigned int delta, const int width) {
...
template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_max<hspmd::bfloat16>(hspmd::bfloat16 x, hspmd::bfloat16 y) {
  #if(__CUDA_ARCH__ >= 800)
  return __hmax(x, y);
  #else
  return static_cast<hspmd::bfloat16>(max(float(x), float(y)));
  #endif
}

template <>
__forceinline__ __device__ hspmd::float16 cuda_max<hspmd::float16>(hspmd::float16 x, hspmd::float16 y) {
  #if(__CUDA_ARCH__ >= 800)
  return __hmax(x, y);
  #else
  return static_cast<hspmd::float16>(max(float(x), float(y)));
  #endif
}
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_min<hspmd::float16>(hspmd::float16 x, hspmd::float16 y) {
  #if(__CUDA_ARCH__ >= 800)
  return __hmin(x, y);
  #else
  return static_cast<hspmd::float16>(min(float(x), float(y)));
  #endif
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_min<hspmd::bfloat16>(hspmd::bfloat16 x, hspmd::bfloat16 y) {
  #if(__CUDA_ARCH__ >= 800)
  return __hmin(x, y);
  #else
  return static_cast<hspmd::bfloat16>(min(float(x), float(y)));
  #endif
}
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_abs<hspmd::float16>(hspmd::float16 x) {
  #if(__CUDA_ARCH__ >= 800)
  return __habs(x);
  #else
  return static_cast<hspmd::float16>(abs(float(x)));
  #endif
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_abs<hspmd::bfloat16>(hspmd::bfloat16 x) {
  #if(__CUDA_ARCH__ >= 800)
  return __habs(x);
  #else
  return static_cast<hspmd::bfloat16>(abs(float(x)));
  #endif
}
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_floor<hspmd::float16>(hspmd::float16 x) {
  return hfloor(x);
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_floor<hspmd::bfloat16>(hspmd::bfloat16 x) {
  #if(__CUDA_ARCH__ >= 800)
  return hfloor(x);
  #else
  return static_cast<hspmd::bfloat16>(floorf(float(x)));
  #endif
}
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_ceil<hspmd::float16>(hspmd::float16 x) {
  return hceil(x);
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_ceil<hspmd::bfloat16>(hspmd::bfloat16 x) {
  #if(__CUDA_ARCH__ >= 800)
  return hceil(x);
  #else
  return static_cast<hspmd::bfloat16>(floorf(float(x)));
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_round<hspmd::float16>(hspmd::float16 x) {
  return static_cast<hspmd::float16>(roundf(float(x)));
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_round<hspmd::bfloat16>(hspmd::bfloat16 x) {
  return static_cast<hspmd::bfloat16>(roundf(float(x)));
}
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_log<hspmd::float16>(hspmd::float16 x) {
  return hlog(x);
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_log<hspmd::bfloat16>(hspmd::bfloat16 x) {
  #if(__CUDA_ARCH__ >= 800)
  return hlog(x);
  #else
  return static_cast<hspmd::bfloat16>(logf(float(x)));
  #endif
}
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_exp<hspmd::float16>(hspmd::float16 x) {
  return hexp(x);
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_exp<hspmd::bfloat16>(hspmd::bfloat16 x) {
  #if(__CUDA_ARCH__ >= 800)
  return hexp(x);
  #else
  return static_cast<hspmd::bfloat16>(expf(float(x)));
  #endif
}
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_erf<hspmd::float16>(hspmd::float16 x) {
  // return herf(x);
  return static_cast<hspmd::float16>(erff(float(x)));
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_erf<hspmd::bfloat16>(hspmd::bfloat16 x) {
  // #if(__CUDA_ARCH__ >= 800)
  // return herf(x);
  // #else
  return static_cast<hspmd::bfloat16>(erff(float(x)));
  // #endif
}
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_sqrt<hspmd::float16>(hspmd::float16 x) {
  return hsqrt(x);
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_sqrt<hspmd::bfloat16>(hspmd::bfloat16 x) {
  #if(__CUDA_ARCH__ >= 800)
  return hsqrt(x);
  #else
  return static_cast<hspmd::bfloat16>(sqrtf(float(x)));
  #endif
}
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_rsqrt<hspmd::float16>(hspmd::float16 x) {
  return hrsqrt(x);
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_rsqrt<hspmd::bfloat16>(hspmd::bfloat16 x) {
  #if(__CUDA_ARCH__ >= 800)
  return hrsqrt(x);
  #else
  return static_cast<hspmd::bfloat16>(rsqrtf(float(x)));
  #endif
}
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_sin<hspmd::float16>(hspmd::float16 x) {
  return hsin(x);
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_sin<hspmd::bfloat16>(hspmd::bfloat16 x) {
  #if(__CUDA_ARCH__ >= 800)
  return hsin(x);
  #else
  return static_cast<hspmd::bfloat16>(sinf(float(x)));
  #endif
}
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_cos<hspmd::float16>(hspmd::float16 x) {
  return hcos(x);
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_cos<hspmd::bfloat16>(hspmd::bfloat16 x) {
  #if(__CUDA_ARCH__ >= 800)
  return hcos(x);
  #else
  return static_cast<hspmd::bfloat16>(cosf(float(x)));
  #endif
}
...
template <>
__forceinline__ __device__ hspmd::float16 cuda_tanh<hspmd::float16>(hspmd::float16 x) {
  return static_cast<hspmd::float16>(tanhf(float(x)));
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_tanh<hspmd::bfloat16>(hspmd::bfloat16 x) {
  return static_cast<hspmd::bfloat16>(tanhf(float(x)));
}
...
__forceinline__ __device__ hspmd::float16 cuda_pow(hspmd::float16 x, double exponent) {
  return static_cast<hspmd::float16>(powf(float(x), float(exponent)));
}

template <>
__forceinline__ __device__ hspmd::float16 cuda_pow<hspmd::float16>(hspmd::float16 x, hspmd::float16 exponent) {
  return static_cast<hspmd::float16>(powf(float(x), float(exponent)));
}

__forceinline__ __device__ hspmd::bfloat16 cuda_pow(hspmd::bfloat16 x, double exponent) {
  return static_cast<hspmd::bfloat16>(powf(float(x), float(exponent)));
}

template <>
__forceinline__ __device__ hspmd::bfloat16 cuda_pow<hspmd::bfloat16>(hspmd::bfloat16 x, hspmd::bfloat16 exponent) {
  return static_cast<hspmd::bfloat16>(powf(float(x), float(exponent)));
}
...
template <>
__forceinline__ __device__ void AtomicAdd<hspmd::float16>(hspmd::float16* address, hspmd::float16 val) {
...
template <>
__forceinline__ __device__ void AtomicAdd<hspmd::bfloat16>(hspmd::bfloat16* address, hspmd::bfloat16 val) {
  #if(__CUDA_ARCH__ >= 800)
